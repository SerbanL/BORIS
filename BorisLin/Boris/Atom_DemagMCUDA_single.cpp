...
Atom_DemagMCUDA_single::Atom_DemagMCUDA_single(Atom_MeshCUDA* paMeshCUDA_, Atom_DemagMCUDA* pDemagMCUDA_, int device_index_) :
	ConvolutionCUDA<Atom_DemagMCUDA_single, DemagKernelCUDA>(),
...
Atom_DemagMCUDA_single::~Atom_DemagMCUDA_single()
{
	for (int idx = 0; idx < mGPU.get_num_devices(); idx++) {
...
BError Atom_DemagMCUDA_single::Initialize(void)
{
	BError error(CLASS_STR(Atom_DemagMCUDA_single));
	...
	normalization.from_cpu(paMeshCUDA->Show_Ms() * N.dim());
	normalization_M.from_cpu(paMeshCUDA->Show_Ms());

	return error;
}

BError Atom_DemagMCUDA_single::UpdateConfiguration(UPDATECONFIG_ cfgMessage)
{
	BError error(CLASS_STR(Atom_DemagMCUDA_single));
...
void Atom_DemagMCUDA_single::UpdateField(void)
{
}
