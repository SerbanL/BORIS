...
BError SMElastic::Initialize(void)
{
	...
	for (int idx = 0; idx < pSMesh->size(); idx++) {
		...
		if ((*pSMesh)[idx]->IsModuleSet(MOD_MELASTIC)) {
			...
			//do not include dormant meshes even if they have elastic module enabled
...
BError SMElastic::UpdateConfiguration(UPDATECONFIG_ cfgMessage)
{
	...
	if (ucfg::check_cfgflags(cfgMessage, UPDATECONFIG_MELASTIC, UPDATECONFIG_MESHADDED, UPDATECONFIG_MESHDELETED, UPDATECONFIG_MODULEADDED, UPDATECONFIG_MODULEDELETED, UPDATECONFIG_SWITCHCUDASTATE)) {
