...
__device__ bool from_notyRegion_in_xRegion_R_Atom(
...
__global__ void Copy_M_Input_xRegion_Atom_kernel(
	...
	cuSZ3& n, cuSZ3& N, cuINT2& xRegion_R, cuINT2& yRegion, int& num_devices)
{
	...
	if (from_notyRegion_in_xRegion_R_Atom(idx, component_idx, idx_out, idx_in, n, N, xRegion_R, yRegion, num_devices))
		M_Input_xRegion[component_idx][idx_out] = M[idx_in];
}

template <typename VECType>
__global__ void Copy_M_Input_xRegion_Atom_halfprecision_kernel(
...
void Atom_DemagMCUDA_single::Copy_M_Input_xRegion(bool half_precision)
{
	if (!half_precision) {

		Copy_M_Input_xRegion_Atom_kernel <<< (nxRegion_R * (n.y - nyRegion) * n.z + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
			(pDemagMCUDA->M.get_deviceobject(device_index), Real_xRegion_arr, cun, cuN, cuxRegion_R, cuyRegion, cunum_devices);
	}
	else {

		Copy_M_Input_xRegion_Atom_halfprecision_kernel <<< (nxRegion_R * (n.y - nyRegion) * n.z + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
			(pDemagMCUDA->M.get_deviceobject(device_index), Real_xRegion_half_arr, cun, cuN, cuxRegion_R, cuyRegion, cunum_devices, normalization_M);
	}
}
