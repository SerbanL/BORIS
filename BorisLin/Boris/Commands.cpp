	...
	if (commands.has_key(command_name)) {
		...
		case CMD_ADDAMESHBCC:
		{
			...
			if (!error) {
				...
				if (!err_hndl.call(error, &SuperMesh::AddMesh, &SMesh, meshName, MESH_ATOM_BCC, meshRect)) {
		...
		case CMD_SETAMESHBCC:
		{
			...
			if (!error) {
				...
				if (!err_hndl.call(error, &SuperMesh::AddMesh, &SMesh, new_meshName, MESH_ATOM_BCC, meshRect)) {
		...
		case CMD_CRYSTAL:
		{
			if (SMesh.IsSuperMeshModuleSet(MODS_SMELASTIC)) {
				...
				optional_meshname_check_focusedmeshdefault(command_fields);
				error = commandSpec.GetParameters(command_fields, meshName, value_text);

				if (!error && SMesh.contains(meshName)) {

					StopSimulation();

					int value = 0;
					if (has_digits_only(value_text)) value = ToNum(value_text);
					else {

						if (value_text == "cubic") value = 0;
						else if (value_text == "hexagonal") value = 1;
						else if (value_text == "tetragonal") value = 2;
						else if (value_text == "trigonal") value = 3;
						else if (value_text == "trigonal_II") value = 4;
						else if (value_text == "orthorhombic") value = 5;
					}

					SMesh[meshName]->CallModuleMethod(&MElastic::SetCrystalSystem, value);

					if (script_client_connected) commSocket.SetSendData(commandSpec.PrepareReturnParameters(SMesh[meshName]->CallModuleMethod(&MElastic::GetCrystalSystem)));
				}
			}
			else error(BERROR_INCORRECTACTION);
		}
			...
			if (!error) {
				...
				if (cudaAvailable) {
					...
					if (status != cudaEnabled) {
						...
						error = SMesh.SwitchCUDAState(status);
						
						if (verbose) {

							if (cudaEnabled) BD.DisplayConsoleMessage("CUDA switched on.");
							else BD.DisplayConsoleMessage("CUDA switched off.");
						}
						UpdateScreen();
					}
				}
		...
		case CMD_CUDACONFIG:
		{
			int disable_p2p, disable_uva;

			error = commandSpec.GetParameters(command_fields, disable_p2p, disable_uva);
			...
			if (!error) {
				...
				if (cudaAvailable) {
					...
					//first switch off if cuda currently on
					bool cudaEnabled_status = cudaEnabled;
					if (cudaEnabled) SMesh.SwitchCUDAState(false);
					
					//now set mGPU configuration
					mGPU.set_disable_p2p(disable_p2p);
					mGPU.set_disable_uva(disable_uva);

					//switch cuda back on if it was on before
					if (cudaEnabled_status) SMesh.SwitchCUDAState(true);

					if (verbose) BD.DisplayConsoleMessage("CUDA configuration changed. P2P is " + std::string(mGPU.is_all_p2p() ? "On" : "Off") + ". UVA is " + std::string(mGPU.is_uva_enabled() ? "On." : "Off."));
					UpdateScreen();
				}
				else error(BERROR_NOTAVAILABLE);
			}
			else if (verbose) BD.DisplayConsoleMessage("P2P is " + std::string(mGPU.is_all_p2p() ? "On" : "Off") + ". UVA is " + std::string(mGPU.is_uva_enabled() ? "On." : "Off."));
#endif

			if (script_client_connected)
				commSocket.SetSendData(commandSpec.PrepareReturnParameters(disable_p2p, disable_uva));
		}
		break;
		...
		case CMD_MGPUAXIS:
		{
			...
			error = commandSpec.GetParameters(command_fields, subvec_axis);

			auto get_axis_text = [&](int subvec_axis) -> std::string
			{
				std::string axis_text = "";
				if (subvec_axis == -1) axis_text = "Automatic (along longest axis)";
				else if (subvec_axis == 0) axis_text = "x axis";
				else if (subvec_axis == 1) axis_text = "y axis";
				else if (subvec_axis == 2) axis_text = "z axis";
			...
			if (!error) {
				...
				if (cudaAvailable) {
					...
					//now set mGPU configuration
					mGPU.set_subvec_axis(subvec_axis);

					//switch cuda back on if it was on before
					if (cudaEnabled_status) SMesh.SwitchCUDAState(true);

					if (verbose) BD.DisplayConsoleMessage("Sub-VEC allocation axis for multiple GPU use changed to : " + get_axis_text(mGPU.get_subvec_axis()));
					UpdateScreen();
				}
				else error(BERROR_NOTAVAILABLE);
			}
			else if (verbose) BD.DisplayConsoleMessage("Sub-VEC allocation axis for multiple GPU is set to : " + get_axis_text(mGPU.get_subvec_axis()));
#endif
		}
		break;

		case CMD_HALFPRECISIONTRANSFER:
		{
			int halfprecision;

			error = commandSpec.GetParameters(command_fields, halfprecision);
			...
			if (!error) {
				...
				if (cudaAvailable) {
					...
					//now set mGPU configuration
					mGPU.set_halfprecision_transfer(halfprecision);

					//switch cuda back on if it was on before
					if (cudaEnabled_status) SMesh.SwitchCUDAState(true);

					if (verbose) if (halfprecision) BD.DisplayConsoleMessage("Half-precision transfer is enabled."); else BD.DisplayConsoleMessage("Half-precision transfer is disabled.");
					UpdateScreen();
				}
				else error(BERROR_NOTAVAILABLE);
			}
			else if (verbose) if (mGPU.get_halfprecision_transfer()) BD.DisplayConsoleMessage("Half-precision transfer is enabled."); else BD.DisplayConsoleMessage("Half-precision transfer is disabled.");
#endif
		}
		break;
		...
		case CMD_SELCUDADEV:
		{
			...
			error = commandSpec.GetParameters(command_fields, devices_string);

			std::vector<std::string> devices_stringvec = split(devices_string);
			std::vector<int> devices = vec_convert<int, std::string>(devices_stringvec);
			...
			if (!error) {
				...
				if (cudaAvailable) {
					...
					if (cudaEnabled) {
						...
						error = SMesh.SwitchCUDAState(true, devices);
					}
					else {
					...
					if (!error && verbose) BD.DisplayConsoleMessage("CUDA device(s) selected.");
		...
		case CMD_GENERATE2DUGRAINS:
		{
			std::string meshName;
			double spacing, variation;
			int seed;

			optional_meshname_check_focusedmeshdefault(command_fields);
			error = commandSpec.GetParameters(command_fields, meshName, spacing, variation, seed);
			if (error == BERROR_PARAMMISMATCH) { error.reset() = commandSpec.GetParameters(command_fields, meshName, spacing, variation); seed = 1; }

			if (!error) {

				if (!err_hndl.qcall(error, &SuperMesh::GenerateUGrains2D, &SMesh, meshName, spacing, variation, seed)) UpdateScreen();
			}
			else if (verbose) PrintCommandUsage(command_name);
		}
		break;

		case CMD_GENERATE3DUGRAINS:
		{
			std::string meshName;
			double spacing, variation;
			int seed;
			...
			if (!error) {
				...
				if (!err_hndl.qcall(error, &SuperMesh::GenerateUGrains3D, &SMesh, meshName, spacing, variation, seed)) UpdateScreen();
			}
			else if (verbose) PrintCommandUsage(command_name);
		}
		break;
		...
		case CMD_SAVEOVF2MAG:
		{
			...
			if (!error) {
				...
				if (SMesh[meshName]->Magnetism_Enabled()) {
					...
					double Ms0 = 1.0;
					if (normalize) {
						if (!SMesh[meshName]->is_atomistic()) Ms0 = dynamic_cast<Mesh*>(SMesh[meshName])->Ms.get0();
						else Ms0 = dynamic_cast<Atom_Mesh*>(SMesh[meshName])->Show_Ms();
					}

					OVF2 ovf2;
					if (!SMesh[meshName]->is_atomistic()) error = ovf2.Write_OVF2_VEC(ScanFileNameData(fileName), dynamic_cast<Mesh*>(SMesh[meshName])->Get_M(), data_type, Ms0);
					else {

						VEC_VC<DBL3>& M1 = dynamic_cast<Atom_Mesh*>(SMesh[meshName])->Get_M1();
						double conversion = MUB / M1.h.dim();
						error = ovf2.Write_OVF2_VEC(ScanFileNameData(fileName), M1, data_type, Ms0 / conversion);
					}
				}
				else err_hndl.show_error(BERROR_NOTMAGNETIC, verbose);
			}
			...
			if (!error) {
				...
				if (SMesh[meshName]->Magnetism_Enabled() && SMesh[meshName]->IsModuleSet(MOD_MELASTIC)) {
					...
					if (fields.size() >= 2) {
						...
						fileName_odiag = trim_leading_spaces(fields[1]) + ".ovf";
		...
		case CMD_DORMANT:
		{
			...
			optional_meshname_check_focusedmeshdefault(command_fields);
			error = commandSpec.GetParameters(command_fields, meshName, dormant);

			if (!error) {

				StopSimulation();

				if (!err_hndl.qcall(error, &SuperMesh::Set_Dormant, &SMesh, dormant, meshName)) UpdateScreen();
			}
			else if (verbose) PrintCommandUsage(command_name);
...
#pragma omp parallel for
		...
		case CMD_TEST:
		{
			...
			dump_params(MESH_ATOM_CUBIC, "AtomisticSC");
			dump_params(MESH_ATOM_BCC, "AtomisticBCC");
			...
			dump_quantities(MESH_ATOM_BCC, meshtypeHandles(MESH_ATOM_BCC));
			*/
		}
		break;
