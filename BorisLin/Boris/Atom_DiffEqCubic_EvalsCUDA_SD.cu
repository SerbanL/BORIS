...
void Atom_DifferentialEquationCubicCUDA::RunSD_Start(void)
{
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
		...
		RunSD_Start_Kernel <<< (paMeshCUDA->M1.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
...
void Atom_DifferentialEquationCubicCUDA::RunSD_BB(void)
{
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
		...
		RunSD_BB_Kernel <<< (paMeshCUDA->M1.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
			(cuaDiffEq.get_deviceobject(mGPU), paMeshCUDA->cuaMesh.get_deviceobject(mGPU));
	}
}
...
void Atom_DifferentialEquationCubicCUDA::RunSD_Advance(bool calculate_mxh, bool calculate_dmdt)
{
	if (calculate_mxh && calculate_dmdt) {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			RunSD_Advance_withReductions_Kernel <<< (paMeshCUDA->M1.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
				(cuaDiffEq.get_deviceobject(mGPU), paMeshCUDA->cuaMesh.get_deviceobject(mGPU));
		}
	}
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			RunSD_Advance_withReduction_mxh_Kernel <<< (paMeshCUDA->M1.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
				(cuaDiffEq.get_deviceobject(mGPU), paMeshCUDA->cuaMesh.get_deviceobject(mGPU));
		}
	}
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			RunSD_Advance_withReduction_dmdt_Kernel <<< (paMeshCUDA->M1.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
				(cuaDiffEq.get_deviceobject(mGPU), paMeshCUDA->cuaMesh.get_deviceobject(mGPU));
		}
	}
	else {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			RunSD_Advance_Kernel <<< (paMeshCUDA->M1.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
				(cuaDiffEq.get_deviceobject(mGPU), paMeshCUDA->cuaMesh.get_deviceobject(mGPU));
		}
	}
}
