...
__global__ void CalculateiDMExchangeCoupling_Atom_kernel(
	...
	cuBReal& energy, bool do_reduction)
{
	...
	if (box_idx < box_sizes.dim()) {
		...
		if (M1_pri.is_not_empty(cell1_idx) && M1_pri.is_cmbnd(cell1_idx)) {
			...
			mesh_pri.update_parameters_mcoarse(cell1_idx, *mesh_pri.pmu_s, mu_s, *mesh_pri.pJ, J, *mesh_pri.pD, D);
			...
			Hexch_D = (D / (MUB_MU0 * mu_s)) * (cuReal3(-m_2.z, 0.0, m_2.x) + cuReal3(-m_m1.z, 0.0, m_m1.x));

			Heff1_pri[cell1_idx] += (Hexch_A + Hexch_D);

			if (do_reduction) {

				cuBReal non_empty_volume = M1_pri.get_nonempty_cells() * M1_pri.h.dim();
				if (non_empty_volume) energy_ = -(cuBReal)MUB_MU0 * M1_pri[cell1_idx] * (Hexch_A + Hexch_D) / (2 * non_empty_volume);
			}
		}
	}
...
// FM to Atomistic
__global__ void CalculateiDMExchangeCoupling_FM_to_Atom_kernel(
...
void Atom_iDMExchangeCUDA::CalculateExchangeCoupling(mcu_val<cuBReal>& energy)
{
		...
		if (pContactingMeshes[idx_pri]->is_atomistic() && pContactingMeshes[idx_sec]->is_atomistic()) {
			...
			if (pMeshBaseCUDA->CurrentTimeStepSolved()) {
				...
				for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
					...
					CalculateiDMExchangeCoupling_Atom_kernel <<< (size + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
			...
			if (pMeshBaseCUDA->CurrentTimeStepSolved()) {
				...
				for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
					...
					CalculateiDMExchangeCoupling_FM_to_Atom_kernel <<< (size + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
