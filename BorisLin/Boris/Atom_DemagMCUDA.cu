	...
	unsigned int NUMREDUCTIONS, unsigned int BLOCKSIZE, unsigned int BLOCKOFFSET)
{
	...
	if (tidx == 0) {
		...
		if (BLOCKOFFSET == 0) M[ijk_m.i + ijk_m.j * n_m.x + ijk_m.k * n_m.x * n_m.y] = pshared_memory[threadIdx.x] * MUB / M.h.dim();
		...
		else M[ijk_m.i + ijk_m.j * n_m.x + ijk_m.k * n_m.x * n_m.y] += pshared_memory[threadIdx.x] * MUB / M.h.dim();
	}
}
	...
	unsigned int BLOCKOFFSET)
{
	...
	Transfer_Moments_to_Magnetization_BlockReduction(
		M, M1, 
		shared_memory, 
		device_index,
...
__global__ void Transfer_Moments_to_Magnetization_Kernel_BlockSize_1(cuVEC<cuReal3>& M, cuVEC_VC<cuReal3>& M1)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
void Atom_DemagMCUDA::Transfer_Moments_to_Magnetization(void)
{
	...
	{
		//find smallest power of 2 greater or equal to remaining_spins (this will be the block size)
		unsigned int pow2 = 1;
	...
	do {
		...
		case 1:
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_1 <<< (M.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
		...
		case 2:
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<MIN_CUDATHREADS, MIN_CUDATHREADS / 2> <<< (M.device_size(mGPU) * 2 + MIN_CUDATHREADS) / MIN_CUDATHREADS, MIN_CUDATHREADS >>>
		...
		case 4:
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<MIN_CUDATHREADS, MIN_CUDATHREADS / 4> <<< (M.device_size(mGPU) * 4 + MIN_CUDATHREADS) / MIN_CUDATHREADS, MIN_CUDATHREADS >>>
					(M.get_deviceobject(mGPU), paMeshCUDA->M1.get_deviceobject(mGPU), mGPU, 0);
			}
			BLOCKOFFSET += 4;
			break;
		case 8:
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<MIN_CUDATHREADS, MIN_CUDATHREADS / 8> <<< (M.device_size(mGPU) * 8 + MIN_CUDATHREADS) / MIN_CUDATHREADS, MIN_CUDATHREADS >>>
					(M.get_deviceobject(mGPU), paMeshCUDA->M1.get_deviceobject(mGPU), mGPU, 0);
			}
			BLOCKOFFSET += 8;
			break;
		case 16:
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<MIN_CUDATHREADS, MIN_CUDATHREADS / 16> <<< (M.device_size(mGPU) * 16 + MIN_CUDATHREADS) / MIN_CUDATHREADS, MIN_CUDATHREADS >>>
					(M.get_deviceobject(mGPU), paMeshCUDA->M1.get_deviceobject(mGPU), mGPU, 0);
			}
			BLOCKOFFSET += 16;
			break;
		case 32:
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<32, 1> <<< M.device_size(mGPU), 32 >>>
					(M.get_deviceobject(mGPU), paMeshCUDA->M1.get_deviceobject(mGPU), mGPU, 0);
			}
			BLOCKOFFSET += 32;
			break;
		case 64:
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<64, 1> <<< M.device_size(mGPU), 64 >>>
					(M.get_deviceobject(mGPU), paMeshCUDA->M1.get_deviceobject(mGPU), mGPU, 0);
			}
			BLOCKOFFSET += 64;
			break;
		case 128:
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<128, 1> <<< M.device_size(mGPU), 128 >>>
					(M.get_deviceobject(mGPU), paMeshCUDA->M1.get_deviceobject(mGPU), mGPU, 0);
			}
			BLOCKOFFSET += 128;
			break;
		case 256:
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<256, 1> <<< M.device_size(mGPU), 256 >>>
					(M.get_deviceobject(mGPU), paMeshCUDA->M1.get_deviceobject(mGPU), mGPU, 0);
			}
			BLOCKOFFSET += 256;
			break;
		...
		case 512:
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<512, 1> <<< M.device_size(mGPU), 512 >>>
