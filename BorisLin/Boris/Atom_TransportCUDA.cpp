	...
	TransportBaseCUDA(pTransport_)
{
	...
	if (!error_on_create) {
		...
		//Poisson objects
		error_on_create = poisson_V.set_pointers(paMeshCUDA);

		error_on_create = poisson_Spin_S.set_pointers(paMeshCUDA, this);
		error_on_create = poisson_Spin_V.set_pointers(paMeshCUDA, this);

		//CMBND objects
		error_on_create = charge_V_cmbnd_funcs_pri.set_pointers(paMeshCUDA);
		error_on_create = charge_V_cmbnd_funcs_sec.set_pointers(paMeshCUDA);

		error_on_create = spin_V_cmbnd_funcs_pri.set_pointers(paMeshCUDA, this);
		error_on_create = spin_V_cmbnd_funcs_sec.set_pointers(paMeshCUDA, this);

		error_on_create = spin_S_cmbnd_funcs_pri.set_pointers(paMeshCUDA, this);
		error_on_create = spin_S_cmbnd_funcs_sec.set_pointers(paMeshCUDA, this);
	}
}
...
BError Atom_TransportCUDA::Initialize(void)
{
	...
	charge_V_cmbnd_funcs_pri.set_thermoelectric_mesh_flag(is_thermoelectric_mesh);
	charge_V_cmbnd_funcs_sec.set_thermoelectric_mesh_flag(is_thermoelectric_mesh);
	charge_V_cmbnd_funcs_pri.set_open_potential_flag(pTransport->IsOpenPotential());
	charge_V_cmbnd_funcs_sec.set_open_potential_flag(pTransport->IsOpenPotential());

	//do we need to enable dM_dt calculation?
	if (Need_dM_dt_Calculation()) {

		if (!dM_dt.assign(paMeshCUDA->h, paMeshCUDA->meshRect, cuReal3(), paMeshCUDA->M1)) return error(BERROR_OUTOFMEMORY_CRIT);
	}
	else {
...
BError Atom_TransportCUDA::UpdateConfiguration(UPDATECONFIG_ cfgMessage)
{
	...
	if (ucfg::check_cfgflags(cfgMessage, UPDATECONFIG_MESHSHAPECHANGE, UPDATECONFIG_MESHCHANGE)) {
		...
		if (paMeshCUDA->V.size_cpu().dim()) {
			...
			success &= paMeshCUDA->V.resize(paMeshCUDA->h_e, paMeshCUDA->meshRect, paMeshCUDA->elC);
		}
		else {

			success &= paMeshCUDA->V.assign(paMeshCUDA->h_e, paMeshCUDA->meshRect, (cuBReal)0.0, paMeshCUDA->elC);
		}
		...
		if (paMeshCUDA->E.size_cpu().dim()) {
			...
			success &= paMeshCUDA->E.resize(paMeshCUDA->h_e, paMeshCUDA->meshRect, paMeshCUDA->elC);
		}
		else {

			success &= paMeshCUDA->E.assign(paMeshCUDA->h_e, paMeshCUDA->meshRect, cuReal3(0.0), paMeshCUDA->elC);
		}
	}
	...
	if (ucfg::check_cfgflags(cfgMessage, UPDATECONFIG_MESHSHAPECHANGE, UPDATECONFIG_MESHCHANGE, UPDATECONFIG_ODE_SOLVER)) {
		...
		if (success && stsolve != STSOLVE_NONE) {
			...
			if (paMeshCUDA->S.size_cpu().dim()) {
				...
				success &= paMeshCUDA->S.resize(paMeshCUDA->h_e, paMeshCUDA->meshRect, paMeshCUDA->elC);
			}
			else {

				success &= paMeshCUDA->S.assign(paMeshCUDA->h_e, paMeshCUDA->meshRect, cuReal3(0.0), paMeshCUDA->elC);
			}
		}
		else {
