...
__global__ void Atom_CalculateCurrentDensity_Spin_Kernel(cuVEC_VC<cuReal3>& Jc, ManagedAtom_MeshCUDA& cuaMesh, TransportCUDA_Spin_V_Funcs& poisson_Spin_V, cuVEC_VC<cuReal3>& dM_dt)
{
...
mcu_VEC_VC(cuReal3)& Atom_TransportCUDA::GetChargeCurrent(void)
{
	...
	if (!pSMeshCUDA->DisabledTransportSolver()) {
		...
		if (stsolve == STSOLVE_NONE) {
			...
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				...
				Atom_CalculateFixedCurrentDensity_Charge_Kernel <<< (paMeshCUDA->elC.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
					(displayVEC_VC.get_deviceobject(mGPU), paMeshCUDA->E.get_deviceobject(mGPU), paMeshCUDA->elC.get_deviceobject(mGPU));
			}
		}
		else {
			...
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				...
				Atom_CalculateCurrentDensity_Spin_Kernel <<< (paMeshCUDA->elC.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
					(displayVEC_VC.get_deviceobject(mGPU), paMeshCUDA->cuaMesh.get_deviceobject(mGPU), poisson_Spin_V.get_deviceobject(mGPU), dM_dt.get_deviceobject(mGPU));
			}
		}
	}
	else {
