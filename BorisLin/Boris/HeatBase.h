...
protected:
	...
	virtual void SetFromGlobalTemperature(VEC<double>& globalTemp) = 0;
...
public:
	...
	//if force_set_uniform is false, then only set base temperature if cT has a spatial variation set, else no change.
	virtual void SetBaseTemperature(double Temperature, bool force_set_uniform = true) = 0;
...
public:
	...
	//set Temp uniformly to base temperature, unless a spatial variation is also specified through the cT mesh parameter
	void SetBaseTemperature(double Temperature, bool force_set_uniform = true) {}
