...
class Atom_Mesh_BCC :
	public Atom_Mesh,
	public ProgramState < Atom_Mesh_BCC,
	std::tuple<
	//Mesh members
	int, int, int,
...
{
#if COMPILECUDA == 1
	friend Atom_Mesh_BCCCUDA;
#endif

private:

	//The set ODE, associated with this BCC atomistic mesh (the ODE type and evaluation method is controlled from SuperMesh)
	Atom_DifferentialEquationBCC meshODE;
	...
	const int coordination_number = 10;

	//number of atomic moments per cell
	const int atoms_per_cell = 2;
...
public:
	...
	Atom_Mesh_BCC(SuperMesh *pSMesh_);

	Atom_Mesh_BCC(Rect meshRect_, DBL3 h_, SuperMesh *pSMesh_);
	...
	{
#if COMPILECUDA == 1
		if (paMeshCUDA) return skyShift.Get_skyshiftCUDA(paMeshCUDA->M1, skyRect);
#endif
	...
	{
#if COMPILECUDA == 1
		if (paMeshCUDA) return skyShift.Get_skypos_diametersCUDA(paMeshCUDA->M1, skyRect);
#endif
	...
	Atom_DifferentialEquationBCC& Get_DifferentialEquation(void) { return meshODE; }
#endif
};
...
public:
	...
	//constructor taking only a SuperMesh pointer (SuperMesh is the owner) only needed for loading : all required values will be set by LoadObjectState method in ProgramState
	Atom_Mesh_BCC(SuperMesh *pSMesh_) :
		Atom_Mesh(MESH_ATOM_BCC, pSMesh_),
		meshODE(this)
	{}
