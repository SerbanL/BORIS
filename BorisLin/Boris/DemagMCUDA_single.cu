...
__device__ bool from_notyRegion_in_xRegion_R(
...
__global__ void Copy_M_Input_xRegion_kernel(
	cuVEC_VC<cuReal3>& M, cuReal3** M_Input_xRegion,
	cuSZ3& n, cuSZ3& N, cuINT2& xRegion_R, cuINT2& yRegion, int& num_devices)
{
	...
	if (from_notyRegion_in_xRegion_R(idx, component_idx, idx_out, idx_in, n, N, xRegion_R, yRegion, num_devices)) 
		M_Input_xRegion[component_idx][idx_out] = M[idx_in];
}

__global__ void Copy_M_Input_xRegion_halfprecision_kernel(
	cuVEC_VC<cuReal3>& M, cuBHalf** M_Input_xRegion_half,
...
//average M and M2 before copying to M_Input_xRegion_arr
__global__ void Copy_M_Input_xRegion_kernel(
	cuVEC_VC<cuReal3>& M, cuVEC_VC<cuReal3>& M2, cuReal3** M_Input_xRegion,
	cuSZ3& n, cuSZ3& N, cuINT2& xRegion_R, cuINT2& yRegion, int& num_devices)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	...
	if (from_notyRegion_in_xRegion_R(idx, component_idx, idx_out, idx_in, n, N, xRegion_R, yRegion, num_devices)) 
		M_Input_xRegion[component_idx][idx_out] = (M[idx_in] + M2[idx_in]) / 2;
}

__global__ void Copy_M_Input_xRegion_halfprecision_kernel(
	cuVEC_VC<cuReal3>& M, cuVEC_VC<cuReal3>& M2, cuBHalf** M_Input_xRegion_half,
	cuSZ3& n, cuSZ3& N, cuINT2& xRegion_R, cuINT2& yRegion, int& num_devices, cuBReal& normalization)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	...
	if (from_notyRegion_in_xRegion_R(idx, component_idx, idx_out, idx_in, n, N, xRegion_R, yRegion, num_devices)) {
		...
		cuReal3 value_in = (M[idx_in] + M2[idx_in]) / (2 * normalization);
...
void DemagMCUDA_single::Copy_M_Input_xRegion(bool half_precision)
{
	if (pMeshCUDA->GetMeshType() == MESH_FERROMAGNETIC) {

		if (!half_precision) {

			Copy_M_Input_xRegion_kernel <<< (nxRegion_R * (n.y - nyRegion) * n.z + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
				(pMeshCUDA->M.get_deviceobject(device_index), Real_xRegion_arr, cun, cuN, cuxRegion_R, cuyRegion, cunum_devices);
		}
		else {

			Copy_M_Input_xRegion_halfprecision_kernel <<< (nxRegion_R * (n.y - nyRegion) * n.z + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
				(pMeshCUDA->M.get_deviceobject(device_index), Real_xRegion_half_arr, cun, cuN, cuxRegion_R, cuyRegion, cunum_devices, normalization_M);
		}
	}
		...
		if (!half_precision) {
			...
			//average M and M2 before copying to M_Input_xRegion_arr
			Copy_M_Input_xRegion_kernel <<< (nxRegion_R * (n.y - nyRegion) * n.z + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
				(pMeshCUDA->M.get_deviceobject(device_index), pMeshCUDA->M2.get_deviceobject(device_index), Real_xRegion_arr, cun, cuN, cuxRegion_R, cuyRegion, cunum_devices);
		}
		else {

			//average M and M2 before copying to M_Input_xRegion_arr
			Copy_M_Input_xRegion_halfprecision_kernel <<< (nxRegion_R * (n.y - nyRegion) * n.z + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
				(pMeshCUDA->M.get_deviceobject(device_index), pMeshCUDA->M2.get_deviceobject(device_index), Real_xRegion_half_arr, cun, cuN, cuxRegion_R, cuyRegion, cunum_devices, normalization_M);
		}
	}
}
