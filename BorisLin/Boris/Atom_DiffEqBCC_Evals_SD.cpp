...
void Atom_DifferentialEquationBCC::RunSD_Start(void)
{
		...
		for (int idx = 0; idx < paMesh->n.dim(); idx++) {
			...
			if (paMesh->M1.is_not_empty(idx)) {
				...
				if (!paMesh->M1.is_skipcell(idx)) {
					...
					DBL3 m = paMesh->M1(sidx, idx) / mu_s;
					DBL3 H = paMesh->Heff1(sidx, idx);
...
void Atom_DifferentialEquationBCC::RunSD_BB(void)
{
		...
		for (int idx = 0; idx < paMesh->n.dim(); idx++) {
			...
			if (paMesh->M1.is_not_empty(idx)) {
				...
				if (!paMesh->M1.is_skipcell(idx)) {
					...
					DBL3 delta_G = (G - sEval0(sidx, idx)) / 1e6;
					...
					//change in M1
					//divide by 1e6 to stop the accumulated value having a large exponent -> both num and denom are divided by same value; if exponent too large when dividing num by denom significant loss of precision can occur.
					DBL3 delta_m = (m - sM1(sidx, idx)) / 1e6;
...
void Atom_DifferentialEquationBCC::RunSD_Advance_withReductions(void)
{
			...
			if (paMesh->M1.is_not_empty(idx)) {
				...
				if (!paMesh->M1.is_skipcell(idx)) {
					...
					if (IsNZ(grel)) {
						...
						double _mxh = GetMagnitude(m ^ H) / (conversion * paMesh->M1(sidx, idx).norm());
					...
					if (calculate_dmdt && IsNZ(grel)) {
						...
						double _dmdt = GetMagnitude(paMesh->M1(sidx, idx) - sM1(sidx, idx)) / (dT * GAMMA * grel * Mnorm * conversion * Mnorm);
...
void Atom_DifferentialEquationBCC::RunSD_Advance(void)
{
	//now we have the stepsize, set new M1 values
	for (int sidx = 0; sidx < paMesh->M1.get_num_sublattices(); sidx++) {
