...
__global__ void Zero_MCAux_SetField_AFMCUDA(cuBReal& aux_real, ManagedMeshCUDA& cuMesh, cuReal3& Ha)
{
...
__global__ void Zero_MCAux_ZeroField_AFMCUDA(cuBReal& aux_real, ManagedMeshCUDA& cuMesh)
{
	if (threadIdx.x == 0) aux_real = 0.0;
	//set applied field for MC
...
__global__ void Iterate_MonteCarloCUDA_Classic_AFM_red_kernel(
	...
	cuBReal mc_cone_angledeg, cuBReal& mc_acceptance_rate, bool do_reduction)
{
	...
	if (spin_idx < n.dim() && M.is_not_empty(spin_idx) && !M.is_skipcell(spin_idx)) {
		...
		cuReal2 energy_delta = cuReal2();
		for (int midx = 0; midx < cuMesh.num_AFM_MCFuncs; midx++)
			if (cuMesh.pAFM_MCFuncs[midx]) energy_delta += (cuMesh.*(cuMesh.pAFM_MCFuncs[midx]))(spin_idx, M_new_A, M2[spin_idx]);
		...
		//1. Find energy change
		energy_delta = cuReal2();
		for (int midx = 0; midx < cuMesh.num_AFM_MCFuncs; midx++)
			if (cuMesh.pAFM_MCFuncs[midx]) energy_delta += (cuMesh.*(cuMesh.pAFM_MCFuncs[midx]))(spin_idx, M[spin_idx], M_new_B);
...
__global__ void Iterate_MonteCarloCUDA_Classic_AFM_black_kernel(
	ManagedMeshCUDA& cuMesh, 
	cuBorisRand<>& prng,
	cuBReal mc_cone_angledeg, cuBReal& mc_acceptance_rate, bool do_reduction)
{
...
cuBReal AFMeshCUDA::Iterate_MonteCarloCUDA_Classic(cuBReal mc_cone_angledeg, double target_acceptance_rate)
{
	if (pHa) {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			Zero_MCAux_SetField_AFMCUDA <<< 1, CUDATHREADS >>> (mc_acceptance_rate(mGPU), cuMesh.get_deviceobject(mGPU), (*pHa)(mGPU));
		}
	}
	else {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			Zero_MCAux_ZeroField_AFMCUDA <<< 1, CUDATHREADS >>> (mc_acceptance_rate(mGPU), cuMesh.get_deviceobject(mGPU));
		}
	}
	...
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
		...
		Iterate_MonteCarloCUDA_Classic_AFM_red_kernel << < (M.device_size(mGPU) / 2 + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> >
	...
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
		...
		Iterate_MonteCarloCUDA_Classic_AFM_black_kernel << < (M.device_size(mGPU) / 2 + CUDATHREADS) / CUDATHREADS, CUDATHREADS >> >
			(cuMesh.get_deviceobject(mGPU),
			prng.get_deviceobject(mGPU),
			mc_cone_angledeg, mc_acceptance_rate(mGPU), mc_acceptance_reduction_counter == 0);
	}
