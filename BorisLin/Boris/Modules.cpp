...
BError Modules::Update_Module_Display_VECs(
	DBL3 h, Rect meshRect, 
	bool Module_Heff_used, bool Module_Energy_used, 
	bool micromagnetic_twosublattice,
	const std::vector<DBL3>& sublattice_vectors)
{
	BError error(CLASS_STR(Modules));
	...
	if (Module_Heff2.linear_size()) {
		...
		if (micromagnetic_twosublattice && Module_Heff_used && !Module_Heff2.resize(h, meshRect)) return error(BERROR_OUTOFMEMORY_CRIT);
		else Module_Heff2.clear();
	}
	else if (micromagnetic_twosublattice && Module_Heff_used) {
	...
	if (Module_energy2.linear_size()) {
		...
		if (micromagnetic_twosublattice && Module_Energy_used && !Module_energy2.resize(h, meshRect)) return error(BERROR_OUTOFMEMORY_CRIT);
		else Module_energy2.clear();
	}
	else if (micromagnetic_twosublattice && Module_Energy_used) {

		if (!Module_energy2.assign(h, meshRect, 0.0)) return error(BERROR_OUTOFMEMORY_CRIT);
	}

	if (Module_Heff.linear_size() && sublattice_vectors.size()) Module_Heff.set_number_of_sublattices(sublattice_vectors);
	if (Module_energy.linear_size() && sublattice_vectors.size()) Module_energy.set_number_of_sublattices(sublattice_vectors);
