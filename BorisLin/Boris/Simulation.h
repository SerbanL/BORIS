...
private:
	...
	//maximum operating GPU temperature (C) - note, at 95 C for most GPUs system will shut down, so best prevent this by setting lower max temperature.
	int max_gpu_temperature = 94;
	//simulation timeout (s) if maximum temperature reached
	int max_temperature_timeout_s = 300;
...
private:
	...
	//true/false return if simulation initialized correctly
	bool PrepareRunSimulation(void);
