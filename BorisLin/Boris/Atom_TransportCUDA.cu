...
void Atom_TransportCUDA::CalculateElectricalConductivity_AMR(void)
{
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
		...
		Atom_CalculateElectricalConductivity_AMR_Kernel <<< (paMeshCUDA->elC.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
...
void Atom_TransportCUDA::CalculateElectricalConductivity_TAMR(void)
{
	if (TAMR_conductivity_equation.is_set()) {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			Atom_CalculateElectricalConductivity_TAMR_Equation_Kernel <<< (paMeshCUDA->elC.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
				(paMeshCUDA->cuaMesh.get_deviceobject(mGPU), TAMR_conductivity_equation.get_x(mGPU));
		}
	}
	else {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			Atom_CalculateElectricalConductivity_TAMR_Kernel <<< (paMeshCUDA->elC.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
				(paMeshCUDA->cuaMesh.get_deviceobject(mGPU));
		}
	}
}
...
void Atom_TransportCUDA::CalculateElectricalConductivity_TAMR_and_AMR(void)
{
	if (TAMR_conductivity_equation.is_set()) {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			Atom_CalculateElectricalConductivity_TAMR_Equation_and_AMR_Kernel <<< (paMeshCUDA->elC.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
				(paMeshCUDA->cuaMesh.get_deviceobject(mGPU), TAMR_conductivity_equation.get_x(mGPU));
		}
	}
	else {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			Atom_CalculateElectricalConductivity_TAMR_and_AMR_Kernel <<< (paMeshCUDA->elC.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
				(paMeshCUDA->cuaMesh.get_deviceobject(mGPU));
		}
	}
}
...
void Atom_TransportCUDA::CalculateElectricalConductivity_NoAMR(void)
{
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
		...
		Atom_CalculateElectricalConductivity_NoAMR_Kernel <<< (paMeshCUDA->elC.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
			(paMeshCUDA->cuaMesh.get_deviceobject(mGPU));
	}
}
...
void Atom_TransportCUDA::CalculateElectricField(bool open_potential)
{
	...
	if (!is_thermoelectric_mesh) {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			Atom_CalculateElectricField_Charge_Kernel <<< (paMeshCUDA->elC.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
				(paMeshCUDA->E.get_deviceobject(mGPU), paMeshCUDA->V.get_deviceobject(mGPU));
		}
	}
	else {
		...
		if (!open_potential) {
			...
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				...
				Atom_CalculateElectricField_Charge_ThermoElectric_Kernel <<< (paMeshCUDA->elC.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
					(paMeshCUDA->cuaMesh.get_deviceobject(mGPU));
			}
		}
		else {
			...
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				...
				Atom_CalculateElectricField_Charge_ThermoElectric_OpenPotential_Kernel <<< (paMeshCUDA->elC.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
					(paMeshCUDA->cuaMesh.get_deviceobject(mGPU), mesh_thermoelectric_net_current(mGPU));
			}
		}
	}
}
...
//charge solver only
void Atom_TransportCUDA::exchange_all_halos_charge(void)
{
	paMeshCUDA->V.exchange_halos();
	paMeshCUDA->elC.exchange_halos();
	if (is_thermoelectric_mesh) paMeshCUDA->Temp.exchange_halos();
}

//spin solver
void Atom_TransportCUDA::exchange_all_halos_spin(void)
{
	paMeshCUDA->V.exchange_halos();
	paMeshCUDA->elC.exchange_halos();
