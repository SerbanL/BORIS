...
void Atom_Mesh_BCC::Compute_TopoChargeDensity(void)
{
	if (M1.linear_size()) {
		...
		if (paMeshCUDA) {
			...
			//compute topological charge density spatial variation in MeshCUDA::auxVEC_cuBReal
			paMeshCUDA->Compute_TopoChargeDensity();

			//now transfer it to displayVEC_SCA ready to display
			paMeshCUDA->copy_auxVEC_cuBReal(displayVEC_SCA);
			return;
		}
#endif
...
double Atom_Mesh_BCC::Show_Transition_Temperature(void)
{
...
DBL2 Atom_Mesh_BCC::FitDomainWall_X(Rect rectangle)
{
	...
	if (paMeshCUDA) {
		...
		mcu_arr<cuReal2>& cu_xy_data = dwPos.getcuda_xy_data_ref();
...
DBL2 Atom_Mesh_BCC::FitDomainWall_Y(Rect rectangle)
{
	if (rectangle.IsNull()) rectangle = meshRect;
...
DBL2 Atom_Mesh_BCC::FitDomainWall_Z(Rect rectangle)
{
	if (rectangle.IsNull()) rectangle = meshRect;
