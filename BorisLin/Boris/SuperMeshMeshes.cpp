...
BError SuperMesh::AddMesh(std::string meshName, MESH_ meshType, Rect meshRect)
{
	...
	//Adding meshes with cuda switched on is problematic. Easiest just switch cuda off, then after mesh added switch it back on.
	...
	case MESH_ATOM_BCC:
		pMesh.push_back(new Atom_Mesh_BCC(meshRect, make_cellsize(DEFAULTATOMCELLSIZE, meshRect), this), meshName);
		break;
	}
