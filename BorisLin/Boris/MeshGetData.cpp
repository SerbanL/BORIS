...
DBL3 Mesh::GetAverageMagnetization(Rect rectangle)
{
	...
	if (pMeshCUDA) {
		...
		if (M.linear_size()) return pMeshCUDA->M.average_nonempty(rectangle);
...
DBL3 Mesh::GetAveragemagnetization2(Rect rectangle)
{
	...
	if (pMeshCUDA) {
		...
		if (M2.linear_size()) return pMeshCUDA->M2.average_nonempty(rectangle);
		else return DBL3(0.0);
	}
#endif
...
DBL2 Mesh::GetmagnetizationMinMax(Rect rectangle)
{
	...
	if (pMeshCUDA) {
		...
		if (M.linear_size()) return pMeshCUDA->M.get_minmax(rectangle);
...
DBL2 Mesh::GetmagnetizationXMinMax(Rect rectangle)
{
	...
	if (pMeshCUDA) {
		...
		if (M.linear_size()) return pMeshCUDA->M.get_minmax_component_x(rectangle);
		else return DBL2(0.0);
	}
#endif
...
DBL2 Mesh::GetmagnetizationYMinMax(Rect rectangle)
{
	...
	if (pMeshCUDA) {
		...
		if (M.linear_size()) return pMeshCUDA->M.get_minmax_component_y( rectangle);
		else return DBL2(0.0);
	}
#endif
...
DBL2 Mesh::GetmagnetizationZMinMax(Rect rectangle)
{
	...
	if (pMeshCUDA) {
		...
		if (M.linear_size()) return pMeshCUDA->M.get_minmax_component_z(rectangle);
		else return DBL2(0.0);
	}
#endif
...
double Mesh::GetAverageElectricalPotential(Rect rectangle)
{
	...
	if (pMeshCUDA) {
		...
		if (V.linear_size()) return pMeshCUDA->V.average_nonempty(rectangle);
		else return 0.0;
	}
#endif
...
DBL3 Mesh::GetAverageSpinAccumulation(Rect rectangle)
{
	...
	if (pMeshCUDA) {
		...
		if (S.linear_size()) return pMeshCUDA->S.average_nonempty(rectangle);
		else return DBL3(0.0);
	}
#endif
...
double Mesh::GetAverageElectricalConductivity(Rect rectangle)
{
	...
	if (pMeshCUDA) {
		...
		if (elC.linear_size()) return pMeshCUDA->elC.average_nonempty(rectangle);
		else return 0.0;
	}
#endif
...
double Mesh::GetAverageTemperature(Rect rectangle)
{
	...
	if (pMeshCUDA) {
		...
		if (Temp.linear_size()) return pMeshCUDA->Temp.average_nonempty(rectangle);
...
double Mesh::GetAverageLatticeTemperature(Rect rectangle)
{
	...
	if (pMeshCUDA) {
		...
		if (Temp_l.linear_size()) return pMeshCUDA->Temp_l.average_nonempty(rectangle);
		else return base_temperature;
	}
#endif
...
DBL3 Mesh::GetAverageMechanicalDisplacement(Rect rectangle)
{
	...
	if (pMeshCUDA) {
		...
		if (u_disp.linear_size()) return pMeshCUDA->u_disp.average_nonempty(rectangle);
		else return DBL3(0.0);
	}
#endif
...
DBL3 Mesh::GetAverageDiagonalStrain(Rect rectangle)
{
	...
	if (pMeshCUDA) {
		...
		if (strain_diag.linear_size()) return pMeshCUDA->strain_diag.average_nonempty(rectangle);
		else return DBL3(0.0);
	}
#endif
...
DBL3 Mesh::GetAverageOffDiagonalStrain(Rect rectangle)
{
	...
	if (pMeshCUDA) {
		...
		if (strain_odiag.linear_size()) return pMeshCUDA->strain_odiag.average_nonempty(rectangle);
		else return DBL3(0.0);
	}
#endif
