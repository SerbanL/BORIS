...
protected:
	...
	mcu_val<cuBReal> aux_real;
	mcu_val<cuReal3> aux_real3;
	mcu_val<size_t> aux_int;

	//storage for extracted mesh profiles
	mcu_arr<cuBReal> profile_storage_sca;
	mcu_arr<cuReal3> profile_storage_vec;

	//auxiliary VECs for computations
	mcu_VEC(cuReal3) auxVEC_cuReal3;
	mcu_VEC(cuBReal) auxVEC_cuBReal;
	...
	mcu_val<cuBReal> mc_acceptance_rate;
	...
	mcu_val<cuReal3> cmc_n;

	//total moment along constrained direction
	mcu_val<cuBReal> cmc_M;
...
public:
	...
	mcu_VEC_VC(cuBReal) Temp;

	//lattice temperature used in many-T models
	mcu_VEC_VC(cuBReal) Temp_l;
	...
	mcu_VEC_VC(cuReal3) u_disp;

	//strain tensor (symmetric):
	//diagonal and off-diagonal components - on n_m, h_m mesh
	//xx, yy, zz
	mcu_VEC_VC(cuReal3) strain_diag;
	//yz, xz, xy
	mcu_VEC_VC(cuReal3) strain_odiag;
...
protected:
	...
	void average_mesh_profile(mcu_VEC(cuBReal)& cuvec_sca, int& num_profile_averages);
	void average_mesh_profile(mcu_VEC_VC(cuBReal)& cuvecvc_sca, int& num_profile_averages);
	void average_mesh_profile(mcu_VEC(cuReal3)& cuvec_vec, int& num_profile_averages);
	void average_mesh_profile(mcu_VEC_VC(cuReal3)& cuvecvc_vec, int& num_profile_averages);
...
public:
	...
	virtual void copy_auxVEC_cuBReal(VEC<double>& displayVEC) = 0;
