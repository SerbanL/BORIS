...
public:
	...
	BError Update_Module_Display_VECs(
		DBL3 h, Rect meshRect, 
		bool Module_Heff_used, bool Module_Energy_used, 
		bool micromagnetic_twosublattice = false,
		const std::vector<DBL3>& = {});
	...
	//Get VECs for display
	mcu_VEC(cuReal3)& Get_Module_HeffCUDA(void) { return pModuleCUDA->Get_Module_Heff(); }
	mcu_VEC(cuReal3)& Get_Module_Heff2CUDA(void) { return pModuleCUDA->Get_Module_Heff2(); }

	//Get VECs for display
	mcu_VEC(cuBReal)& Get_Module_EnergyCUDA(void) { return pModuleCUDA->Get_Module_Energy(); }
	mcu_VEC(cuBReal)& Get_Module_Energy2CUDA(void) { return pModuleCUDA->Get_Module_Energy2(); }
#endif
