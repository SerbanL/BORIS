...
#if COMPILECUDA == 1
#include "Atom_DiffEqBCCCUDA.h"
#endif

class Atom_DifferentialEquationBCC :
	public Atom_DifferentialEquation
{

public:

	Atom_DifferentialEquationBCC(Atom_Mesh_BCC *paMesh);
	~Atom_DifferentialEquationBCC();
	...
	//---------------------------------------- EQUATIONS : Atom_DiffEqBCC_Equations.cpp and Atom_DiffEqBCC_SEquations.cpp

	//------- Single lattice versions (not used for simple cubic)
	...
	//------- Multiple sub-lattice versions

	//Landau-Lifshitz-Gilbert equation
	DBL3 LLG(int sidx, int idx);

	//Landau-Lifshitz-Gilbert equation but with no precession term and damping set to 1 : faster relaxation for static problems
	DBL3 LLGStatic(int sidx, int idx);

	//Landau-Lifshitz-Gilbert equation with Zhang-Li STT
	DBL3 LLGSTT(int sidx, int idx);

	//Stochastic Landau-Lifshitz-Gilbert equation
	DBL3 SLLG(int sidx, int idx);

	//Stochastic Landau-Lifshitz-Gilbert equation with Zhang-Li STT
	DBL3 SLLGSTT(int sidx, int idx);

	//---------------------------------------- OTHERS : Atom_DiffEqCubic.cpp
...
public:
	...
	//Landau-Lifshitz-Gilbert equation
	DBL3 LLG(int sidx, int idx) { return DBL3(); }

	//Landau-Lifshitz-Gilbert equation but with no precession term and damping set to 1 : faster relaxation for static problems
	DBL3 LLGStatic(int sidx, int idx) { return DBL3(); }

	//Landau-Lifshitz-Gilbert equation with Zhang-Li STT
	DBL3 LLGSTT(int sidx, int idx) { return DBL3(); }

	//Stochastic Landau-Lifshitz-Gilbert equation
	DBL3 SLLG(int sidx, int idx) { return DBL3(); }

	//Stochastic Landau-Lifshitz-Gilbert equation with Zhang-Li STT
	DBL3 SLLGSTT(int sidx, int idx) { return DBL3(); }
