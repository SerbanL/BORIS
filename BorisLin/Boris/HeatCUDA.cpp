	...
	HeatBaseCUDA()
{
	...
	if (!error_on_create) {
		...
		error_on_create = temp_cmbnd_funcs_pri.set_pointers(pMeshCUDA);
		error_on_create = temp_cmbnd_funcs_sec.set_pointers(pMeshCUDA);
	}
}
...
BError HeatCUDA::UpdateConfiguration(UPDATECONFIG_ cfgMessage)
{
	...
	if (ucfg::check_cfgflags(cfgMessage, UPDATECONFIG_MESHSHAPECHANGE, UPDATECONFIG_MESHCHANGE, UPDATECONFIG_MODULEADDED, UPDATECONFIG_MODULEDELETED, UPDATECONFIG_HEAT_MODELTYPE)) {
		...
		if (pMeshCUDA->M.size_cpu().dim()) {
			...
			if (success) {
				...
				if (pMeshCUDA->Temp.size_cpu().dim()) {
					...
					success = pMeshCUDA->Temp.resize(pMesh->h_t, pMesh->meshRect, pMeshCUDA->elC);

					//lattice temperature for many-temperature models
					if (pHeat->tmtype == TMTYPE_2TM) success &= pMeshCUDA->Temp_l.resize(pMesh->h_t, pMesh->meshRect, pMeshCUDA->Temp);
				}
				else {
		...
		if (success) success = heatEq_RHS.resize(pMesh->h_t, pMesh->meshRect);
