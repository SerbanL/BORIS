...
double SurfExchange::UpdateField(void)
{
	...
	auto calculate_mm_coupling = [](
		VEC_VC<DBL3>& M, int cell_idx, 
		Mesh* pMeshCoupled, DBL3 cell_rel_pos, 
		double Ms, double J1, double J2, double cell_size,
		DBL3& Hsurfexch, double& cell_energy) -> void 
	{
		...
		if (pMeshCoupled->GetMeshType() == MESH_FERROMAGNETIC) {
			...
			DBL3 m_i = normalize(M[cell_idx]);

			double dot_prod = m_i * m_j;

			//total surface exchange field in coupling cells, including bilinear and biquadratic terms
			Hsurfexch += (m_j / (MU0 * Ms * cell_size)) * (J1 + 2 * J2 * dot_prod);
			cell_energy += (-1 * J1 - 2 * J2 * dot_prod) * dot_prod / cell_size;
		}
			...
			Hsurfexch += (m_j1 / (MU0 * Ms * cell_size)) * J1;
			Hsurfexch += (m_j2 / (MU0 * Ms * cell_size)) * J2;
			cell_energy += (-J1 * (m_i * m_j1) - J2 * (m_i * m_j2)) / cell_size;
		}
	};
...
#pragma omp parallel for
		for (int idx = 0; idx < pMesh->M.linear_size(); idx++) {
			...
			if (pMesh->M.is_not_empty(idx) && bulk_coupling_mask[idx] != INT3()) {
				...
				double J1 = pMesh->J1;
				double J2 = pMesh->J2;
				pMesh->update_parameters_mcoarse(idx, pMesh->Ms, Ms, pMesh->J1, J1, pMesh->J2, J2);
...
double SurfExchange::Get_EnergyChange(int spin_index, DBL3 Mnew)
{
	...
	auto calculate_mm_coupling = [](
		VEC_VC<DBL3>& M, DBL3 Mnew, int spin_index,
		Mesh* pMeshCoupled, DBL3 cell_rel_pos,
		...
		double& energy_old, double& energy_new) -> void
	{
		if (pMeshCoupled->GetMeshType() == MESH_FERROMAGNETIC) {
			...
			DBL3 m_j = normalize(pMeshCoupled->M[cell_rel_pos]);
			DBL3 m_i = normalize(M[spin_index]);
		...
		double& energy_old, double& energy_new) -> void
	{
		...
		for (int ai = acells.s.i; ai < acells.e.i; ai++) {
			for (int aj = acells.s.j; aj < acells.e.j; aj++) {
				for (int ak = acells.s.k; ak < acells.e.k; ak++) {
					...
					int acell_idx = ai + aj * M1.n.x + aj * M1.n.x * M1.n.y;
		...
		//total surface exchange field in coupling cells, including bilinear and biquadratic terms
		m_j = normalize(m_j);
		double dot_prod = normalize(M[spin_index]) * m_j;
		energy_old += (-1 * J1 - 2 * J2 * dot_prod) * dot_prod / cell_size;
