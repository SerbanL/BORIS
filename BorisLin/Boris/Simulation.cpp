...

{
	...
	if (cudaAvailable) {
		...
		if (cudaDevice < 0) {
			...
			if (cudaDeviceVersions[cudaDevice].first == __CUDA_ARCH__) select_cuda_devices({ cudaDevice });
		}
	...
	commands.insert(CMD_ADDAMESHBCC, CommandSpecifier(CMD_ADDAMESHBCC), "addameshbcc");
	commands[CMD_ADDAMESHBCC].usage = "[tc0,0.5,0,1/tc]USAGE : <b>addameshbcc</b> <i>name rectangle</i>";
	commands[CMD_ADDAMESHBCC].limits = { { Any(), Any() }, { Rect(DBL3(-MAXSIMSPACE / 2), DBL3(-MAXSIMSPACE / 2) + DBL3(MINMESHSPACE)), Rect(DBL3(-MAXSIMSPACE / 2), DBL3(MAXSIMSPACE / 2)) } };
	commands[CMD_ADDAMESHBCC].descr = "[tc0,0.5,0.5,1/tc]Add an atomistic mesh with body-centred cubic structure, with given name and rectangle (m). The rectangle can be specified as: <i>sx sy sz ex ey ez</i> for the start and end points in Cartesian coordinates, or as: <i>ex ey ez</i> with the start point as the origin.";
	commands[CMD_ADDAMESHBCC].unit = "m";

	commands.insert(CMD_SETAMESHBCC, CommandSpecifier(CMD_SETAMESHBCC), "setameshbcc");
	commands[CMD_SETAMESHBCC].usage = "[tc0,0.5,0,1/tc]USAGE : <b>setameshbcc</b> <i>name rectangle</i>";
	commands[CMD_SETAMESHBCC].limits = { { Any(), Any() }, { Rect(DBL3(-MAXSIMSPACE / 2), DBL3(-MAXSIMSPACE / 2) + DBL3(MINMESHSPACE)), Rect(DBL3(-MAXSIMSPACE / 2), DBL3(MAXSIMSPACE / 2)) } };
	commands[CMD_SETAMESHBCC].descr = "[tc0,0.5,0.5,1/tc]Set a single atomistic mesh (deleting all other meshes) with body-centred cubic structure, with given name and rectangle (m). The rectangle can be specified as: <i>sx sy sz ex ey ez</i> for the start and end points in Cartesian coordinates, or as: <i>ex ey ez</i> with the start point as the origin.";
	commands[CMD_SETAMESHBCC].unit = "m";
	...
	commands[CMD_SHIFTDIPOLE].usage = "[tc0,0.5,0,1/tc]USAGE : <b>shiftdipole</b> <i>meshname shift</i>";
	...
	commands.insert(CMD_CRYSTAL, CommandSpecifier(CMD_CRYSTAL), "crystal");
	commands[CMD_CRYSTAL].usage = "[tc0,0.5,0,1/tc]USAGE : <b>crystal</b> <i>(meshname) value_or_string</i>";
	commands[CMD_CRYSTAL].limits = { {Any(), Any()},  { Any(), Any() } };
	commands[CMD_CRYSTAL].descr = "[tc0,0.5,0.5,1/tc]Set crystal system to be used for magneto-elastic module in named mesh (focused mesh if not specified). Specify using numerical value or string. 0: cubic (default), 1: hexagonal (not yet implemented), 2: tetragonal (not yet implemented), 3: trigonal, 4: trigonal_II (not yet implemented), 5: orthorhombic (not yet implemented).";
	commands[CMD_CRYSTAL].return_descr = "[tc0,0.5,0,1/tc]Script return values: <i>value</i>";
	...
	commands[CMD_SELCUDADEV].usage = "[tc0,0.5,0,1/tc]USAGE : <b>selectcudadevice</b> <i>device...</i>";
	...
	commands[CMD_CUDACONFIG].descr = "[tc0,0.5,0.5,1/tc]ADVANCED USER SETTING. Change CUDA configuration for multi-GPU computations. disable_p2p: set to 1 to disable P2P memory transfers (only for testing purposes, leave as 0 for normal use). disable_uva: set to 1 to disable UVA; multi-GPU simulations with more than 2 GPUs can be much slower if UVA is enabled, but with 2 GPUs an improvement could be obtained (default value of 1 used); NOTE: if UVA is disabled, then only block memory transfers are used between devices before computations (e.g. halos), whereas with UVA, memory transfers are done as needed inside compute kernels directly. Multi-mesh computations require UVA, for which P2P must also be available and enabled.";
	commands[CMD_CUDACONFIG].limits = { { int(0), int(1) }, { int(0), int(1) } };
	commands[CMD_CUDACONFIG].return_descr = "[tc0,0.5,0,1/tc]Script return values: <i>disable_p2p disable_uva</i>";

	commands.insert(CMD_MGPUAXIS, CommandSpecifier(CMD_MGPUAXIS), "mgpuaxis");
	commands[CMD_MGPUAXIS].usage = "[tc0,0.5,0,1/tc]USAGE : <b>mgpuaxis</b> <i>subvec_axis</i>";
	commands[CMD_MGPUAXIS].descr = "[tc0,0.5,0.5,1/tc]ADVANCED USER SETTING. Change axis for memory allocation when using multiple GPUs. -1 : automatic, memory will be allocated along longest axis, 0 : along x, 1 : along y, 2 : along z.";
	commands[CMD_MGPUAXIS].limits = { { int(-1), int(2) } };

	commands.insert(CMD_HALFPRECISIONTRANSFER, CommandSpecifier(CMD_HALFPRECISIONTRANSFER), "halfprecisiontransfer");
	commands[CMD_HALFPRECISIONTRANSFER].usage = "[tc0,0.5,0,1/tc]USAGE : <b>halfprecisiontransfer</b> <i>status</i>";
	commands[CMD_HALFPRECISIONTRANSFER].descr = "[tc0,0.5,0.5,1/tc]ADVANCED USER SETTING. When using multiple GPUs can choose to convert data to half-precision before transferring between devices for demag field calculation. This is only useful when data transfer is the bottleneck (e.g. more than 2 GPUs with low bandwidth connections).";
	commands[CMD_HALFPRECISIONTRANSFER].limits = { { int(0), int(1) } };
	...
	commands.insert(CMD_GENERATE2DUGRAINS, CommandSpecifier(CMD_GENERATE2DUGRAINS), "generate2dugrains");
	commands[CMD_GENERATE2DUGRAINS].usage = "[tc0,0.5,0,1/tc]USAGE : <b>generate2dugrains</b> <i>(meshname) spacing variation (seed)</i>";
	commands[CMD_GENERATE2DUGRAINS].limits = { { Any(), Any() }, { double(0), Any() }, { double(0), Any() }, { int(0), Any() } };
	commands[CMD_GENERATE2DUGRAINS].unit = "m";
	commands[CMD_GENERATE2DUGRAINS].descr = "[tc0,0.5,0.5,1/tc]Generate 2D Voronoi cells in the xy plane on a regular grid at given spacing and with random variation in given mesh (focused mesh if not specified). This results in uniform grains, with uniformity controlled by variation parameter. The seed is used for the pseudo-random number generator, 1 by default.";

	commands.insert(CMD_GENERATE3DUGRAINS, CommandSpecifier(CMD_GENERATE3DUGRAINS), "generate3dugrains");
	commands[CMD_GENERATE3DUGRAINS].usage = "[tc0,0.5,0,1/tc]USAGE : <b>generate3dgrains</b> <i>(meshname) spacing variation (seed)</i>";
	commands[CMD_GENERATE3DUGRAINS].limits = { { Any(), Any() }, { double(0), Any() }, { double(0), Any() }, { int(0), Any() } };
	commands[CMD_GENERATE3DUGRAINS].unit = "m";
	commands[CMD_GENERATE3DUGRAINS].descr = "[tc0,0.5,0.5,1/tc]Generate 3D Voronoi cells on a regular grid at given spacing and with random variation in given mesh (focused mesh if not specified). This results in uniform grains, with uniformity controlled by variation parameter. The seed is used for the pseudo-random number generator, 1 by default.";
	...
	commands[CMD_LOADOVF2STRAIN].limits = { { Any(), Any() }, { Any(), Any() } };
	...
	commands.insert(CMD_DORMANT, CommandSpecifier(CMD_DORMANT), "dormant");
	commands[CMD_DORMANT].usage = "[tc0,0.5,0,1/tc]USAGE : <b>dormant</b> <i>(meshname) status</i>";
	...
	meshtypeHandles.push_back("Atomistic_BCC", MESH_ATOM_BCC);
