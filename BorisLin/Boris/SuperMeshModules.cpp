		...
		for (int idx = 0; idx < (int)superMeshExclusiveModules[moduleId].size(); idx++) {
			...
			if (meshName == superMeshHandle) {
				...
				for (int idxMesh = 0; idxMesh < (int)pMesh.size(); idxMesh++) {
					...
					//the exception is if the mesh is dormant - keep the exclusive module as it may be needed (e.g. Track shifting algorithm with demag / sdemag combination)
					if (!pMesh[idxMesh]->Is_Dormant()) pMesh[idxMesh]->DelModule(module);
				}
			}
