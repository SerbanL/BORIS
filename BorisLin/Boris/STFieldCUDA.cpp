		...
		if (!error) {
			...
			for (int idx = 0; idx < pSTField->paMesh_Bot.size(); idx++) {
				...
				for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
					...
					pMeshAtom_Bot.push_back(mGPU, pSTField->paMesh_Bot[idx]->paMeshCUDA->cuaMesh.get_managed_object(mGPU));
				}
			}
			...
			for (int idx = 0; idx < pSTField->paMesh_Top.size(); idx++) {
				...
				for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
					...
					pMeshAtom_Top.push_back(mGPU, pSTField->paMesh_Top[idx]->paMeshCUDA->cuaMesh.get_managed_object(mGPU));
				}
			}
