	...
	if (step == 0) {
		...
		if (calculate_mxh) {
			...
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				...
				RunTEuler_Step0_withReductions_Kernel <<< (paMeshCUDA->M1.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
			...
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				...
				RunTEuler_Step0_Kernel <<< (paMeshCUDA->M1.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
					(cuaDiffEq.get_deviceobject(mGPU), paMeshCUDA->cuaMesh.get_deviceobject(mGPU));
			}
		}
	}
		...
		if (calculate_dmdt) {
			...
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				...
				RunTEuler_Step1_withReductions_Kernel <<< (paMeshCUDA->M1.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
					(cuaDiffEq.get_deviceobject(mGPU), paMeshCUDA->cuaMesh.get_deviceobject(mGPU));
			}
		}
		else {
			...
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				...
				RunTEuler_Step1_Kernel <<< (paMeshCUDA->M1.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
					(cuaDiffEq.get_deviceobject(mGPU), paMeshCUDA->cuaMesh.get_deviceobject(mGPU));
			}
		}
	}
