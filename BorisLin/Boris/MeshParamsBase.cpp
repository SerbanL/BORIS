...
void MeshParamsBase::set_meshparam_value(PARAM_ paramID, std::string value_text)
{
	...
	{
		int num_components = MatP_object.num_components();

		std::vector<std::string> fields = split(value_text);

		std::vector<decltype(MatP_object.get0())> separate_values;
		int fields_index = 0;
		while (fields_index + num_components <= fields.size()) {

			std::string value_string = combine(subvec(fields, fields_index, fields_index + num_components), " ");	
			decltype(MatP_object.get0()) value = ToNum(value_string, unit);
			separate_values.push_back(value);

			fields_index += num_components;
		}

		if (separate_values.size() == 1) MatP_object = separate_values[0];
		else MatP_object = separate_values;
	};
