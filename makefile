...
configure:
	...
	$(file >> BorisCUDALib/cuBLib_Flags.h,)
	$(file >> BorisCUDALib/cuBLib_Flags.h,)
	$(file >> BorisCUDALib/cuBLib_Flags.h,#define COMPILE_NVML_CODE 1)
	$(file > arch.txt,$(arch))
...
install:
	...
	g++ $(OBJ_DIR)/*.o $(CUOBJ_DIR)/*.o -fopenmp -lpython$(python) -ltbb /usr/lib/x86_64-linux-gnu/libfftw3.so.3 -lX11 -L/usr/local/cuda-$(cuda)/targets/x86_64-linux/lib/ -L/usr/local/cuda-$(cuda)/targets/x86_64-linux/lib/stubs/ -L/usr/lib/x86_64-linux-gnu/ -lnvidia-ml -lcudart -lcufft -lcudadevrt -o BorisLin
