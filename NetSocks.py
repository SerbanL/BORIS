...
class NSClient:
    ...
    def Get_Data_Columns(self, fileName, column_indexes = '', separator = '\t'):
        ...
        f = open(fileName, 'r')
        if separator == '':
            rows = [[float(number) for number in row.rstrip().split()] for row in f.readlines() if self.can_convert(row, separator)]
        else:
    ...
    def Shift_Rect(self, rect, shift):
        """Shift rect (6 coordinates start and end xyz) by shift (3 coordinates xyz)"""
        return [rect[0] + shift[0], rect[1] + shift[1], rect[2] + shift[2], rect[3] + shift[0], rect[4] + shift[1], rect[5] + shift[2]]
    
    def Rect_Start(self, rect):
        """From rect (6 coordinates start and end xyz) get start coordinates only"""
        return [rect[0], rect[1], rect[2]]
                
    def Rect_End(self, rect):
        """From rect (6 coordinates start and end xyz) get end coordinates only"""
        return [rect[3], rect[4], rect[5]]
    
    def Cellsize(self, n, rect):
        """From rectangle and number of cells return cellsize"""
        return [(rect[3]-rect[0])/n[0], (rect[4]-rect[1])/n[1], (rect[5]-rect[2])/n[2]]
    
    def Get_Subvec(self, vec, n, rect, rel_subrect):
        """From vec with n cells and rect, extract a sub-vec defined by sub_rect with relative coordinates"""
        
        #cellsize
        h = [(rect[3] - rect[0]) / n[0], (rect[4] - rect[1]) / n[1], (rect[5] - rect[2]) / n[2]]
        
        #number of cell in sub-vec
        ns = [int(np.round((rel_subrect[3] - rel_subrect[0])/h[0])), int(np.round((rel_subrect[4] - rel_subrect[1])/h[1])), int(np.round((rel_subrect[5] - rel_subrect[2])/h[2]))]
        
        svec = np.zeros((ns[0]*ns[1]*ns[2], 3))
        
        i0 = int(np.round((rel_subrect[0])/h[0]))
        j0 = int(np.round((rel_subrect[1])/h[1]))
        k0 = int(np.round((rel_subrect[2])/h[2]))
        
        for i, j, k in product(range(ns[0]), range(ns[1]), range(ns[2])):
            
            sidx = i + j * ns[0] + k * ns[0]*ns[1]
            idx = (i + i0) + (j + j0) * n[0] + (k + k0) * n[0]*n[1]
            
            svec[sidx] = vec[idx]
            
        return svec, ns, self.Shift_Rect(rel_subrect, self.Rect_Start(rect))
    ...
    def Read_OVF2(self, fileName, rel_rect = []):
        """Return a numpy array, integer number of cells, and mesh rectangle, from OVF2 file.
           If rel_rect is specified then read only a portion in given relative coordinates rectangle.
        """
        vec = np.ndarray(0)
        ...
        def return_vec():
            ...
            if rel_rect != []:
                return self.Get_Subvec(vec, n, meshRect, rel_rect)
            else:
    ...
    def Combine_OVF2(self, ovf_files, rel_rect = [], h = [1, 1, 1]):
        ...
        If rel_rect specified, then obtain array only in given rectangle (specified with relative coordinates).
        """
        ...
        n = [int(np.round((srect[3] - srect[0])/h[0])), int(np.round((srect[4] - srect[1])/h[1])), int(np.round((srect[5] - srect[2])/h[2]))]
        ...
        for vec_info in vecs_info:
            ...
            for i, j, k in product(range(vn[0]), range(vn[1]), range(vn[2])):
                ...
                ijk_s = [int(np.round((abs_pos_s[0] - srect[0]) / h[0])), int(np.round((abs_pos_s[1] - srect[1]) / h[1])), int(np.round((abs_pos_s[2] - srect[2]) / h[2]))]
                ijk_e = [int(np.round((abs_pos_e[0] - srect[0]) / h[0])), int(np.round((abs_pos_e[1] - srect[1]) / h[1])), int(np.round((abs_pos_e[2] - srect[2]) / h[2]))]
        ...
        if rel_rect != []:
            return self.Get_Subvec(vec, n, srect, rel_rect)
        else:
            return vec, n, srect
        ...
        if embedded_mode:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                ...
                try:
                    ...
                    #receive response to run command and 'stopped' signal
                    data = str(sock.recv(self.maxLenMessage), 'utf-8')
                    fields = data.split('\t')
                    if len(fields) == 2: str(sock.recv(self.maxLenMessage), 'utf-8')
                    if self.script_verbose: print('RX :')
                except:
    ...
    def clearglobaltemp(self, bufferCommand = False):
    	if not bufferCommand: return self.SendCommand("clearglobaltemp")
    	self.SendCommand("buffercommand", ["clearglobaltemp"])
    ...
    def copymeshdata(self, meshname_from = '', meshname_to = '', bufferCommand = False):
        if issubclass(type(meshname_from), self.Mesh): meshname_from = meshname_from.meshname
        if issubclass(type(meshname_to), self.Mesh): meshname_to = meshname_to.meshname
    ...
    def crystal(self, meshname = '', value_or_string = '', bufferCommand = False):
    	if issubclass(type(meshname), self.Mesh): meshname = meshname.meshname
    	if not bufferCommand: return self.SendCommand("crystal", [meshname, value_or_string])
    	self.SendCommand("buffercommand", ["crystal", meshname, value_or_string])
    ...
    def cudaconfig(self, disable_p2p = '', disable_uva = '', bufferCommand = False):
    	if not bufferCommand: return self.SendCommand("cudaconfig", [disable_p2p, disable_uva])
    	self.SendCommand("buffercommand", ["cudaconfig", disable_p2p, disable_uva])
    ...
    def dormant(self, meshname = '', status = '', bufferCommand = False):
    	if issubclass(type(meshname), self.Mesh): meshname = meshname.meshname
    	if not bufferCommand: return self.SendCommand("dormant", [meshname, status])
    	self.SendCommand("buffercommand", ["dormant", meshname, status])
    ...
    def generate2dugrains(self, meshname = '', spacing = '', variation = '', seed = '', bufferCommand = False):
    	if issubclass(type(meshname), self.Mesh): meshname = meshname.meshname
    	if not bufferCommand: return self.SendCommand("generate2dugrains", [meshname, spacing, variation, seed])
    	self.SendCommand("buffercommand", ["generate2dugrains", meshname, spacing, variation, seed])
    ...
    def generate3dugrains(self, meshname = '', spacing = '', variation = '', seed = '', bufferCommand = False):
    	if issubclass(type(meshname), self.Mesh): meshname = meshname.meshname
    	if not bufferCommand: return self.SendCommand("generate3dugrains", [meshname, spacing, variation, seed])
    	self.SendCommand("buffercommand", ["generate3dugrains", meshname, spacing, variation, seed])
    ...
    def globalfieldvelocity(self, velocity = '', clipping = '', bufferCommand = False):
    	if not bufferCommand: return self.SendCommand("globalfieldvelocity", [velocity, clipping])
    	self.SendCommand("buffercommand", ["globalfieldvelocity", velocity, clipping])
    
    def globaltempvelocity(self, velocity = '', clipping = '', bufferCommand = False):
    	if not bufferCommand: return self.SendCommand("globaltempvelocity", [velocity, clipping])
    	self.SendCommand("buffercommand", ["globaltempvelocity", velocity, clipping])
    ...
    def mgpuaxis(self, subvec_axis = '', bufferCommand = False):
    	if not bufferCommand: return self.SendCommand("mgpuaxis", [subvec_axis])
    	self.SendCommand("buffercommand", ["mgpuaxis", subvec_axis])
    ...
    def shiftglobaltemp(self, shift = '', bufferCommand = False):
    	if not bufferCommand: return self.SendCommand("shiftglobaltemp", [shift])
    	self.SendCommand("buffercommand", ["shiftglobaltemp", shift])
    ...
    def trackshifting(self, meshname = '', sim_meshname = '', velocity = '', clipping = '', further_meshnames = '', bufferCommand = False):
        if issubclass(type(meshname), self.Mesh): meshname = meshname.meshname
        if issubclass(type(sim_meshname), self.Mesh): sim_meshname = sim_meshname.meshname
        if isinstance(further_meshnames, list):
            further_meshnames = [mesh.meshname for mesh in further_meshnames if issubclass(type(mesh), self.Mesh)]
        if not bufferCommand: return self.SendCommand("trackshifting", [meshname, sim_meshname, velocity, clipping, further_meshnames])
        self.SendCommand("buffercommand", ["trackshifting", meshname, sim_meshname, velocity, clipping, further_meshnames])
    ...
    def setsavedata(self, filename, *args):
        ...
        for (data, idxdata) in zip(args, range(len(args))):
            for idx in range(len(data)):
                ...
                if idx == 1: 
                    if isinstance(args[idxdata], str): meshname = data[idx]
    ...
    class Parameters_Ferromagnet:
        ...
        def __init__(self, ns, meshname):
            ...
            self.MEc2 = ns.Param(ns, 'MEc2', meshname)
            self.mMEc2 = ns.Param(ns, 'mMEc2', meshname)
            self.MEc3 = ns.Param(ns, 'MEc3', meshname)
            self.mMEc3 = ns.Param(ns, 'mMEc3', meshname)
            self.Ym = ns.Param(ns, 'Ym', meshname)
            self.Pr = ns.Param(ns, 'Pr', meshname)
            self.cC = ns.Param(ns, 'cC', meshname)
            self.cC2 = ns.Param(ns, 'cC2', meshname)
            self.cC3 = ns.Param(ns, 'cC3', meshname)
            self.cCs = ns.Param(ns, 'cCs', meshname)
    ...
    class Mesh:
        ...
        def crystal(self, value_or_string = ''):
        	return self.ns.crystal(self.meshname, value_or_string)
        ...
        def dormant(self, status = ''):
        	return self.ns.dormant(self.meshname, status)
        ...
        def generate2dugrains(self, spacing = '', variation = '', seed = ''):
        	return self.ns.generate2dugrains(self.meshname, spacing, variation, seed)
        
        def generate3dgrains(self, spacing = '', seed = ''):
        	return self.ns.generate3dgrains(self.meshname, spacing, seed)
        
        def generate3dugrains(self, spacing = '', variation = '', seed = ''):
        	return self.ns.generate3dugrains(self.meshname, spacing, variation, seed)
