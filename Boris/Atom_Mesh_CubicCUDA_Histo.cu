...
cuReal3 Atom_Mesh_CubicCUDA::GetThermodynamicAverageMagnetization(cuRect rectangle)
{
	...
	if (pHa) {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			GetThermodynamicAverageMagnetization_kernel <<< (M1.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
				(cuaMesh.get_deviceobject(mGPU), cuaModules(mGPU), cuaNumModules(mGPU), (*pHa)(mGPU), rectangle, aux_real(mGPU), aux_real3(mGPU));
		}
	}
	//No field (or rather Atom_ZeemanCUDA module not added)
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			GetThermodynamicAverageMagnetization_kernel <<< (M1.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
				(cuaMesh.get_deviceobject(mGPU), cuaModules(mGPU), cuaNumModules(mGPU), Ha(mGPU), rectangle, aux_real(mGPU), aux_real3(mGPU));
		}
	}
	...
	if (pHa) {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			Get_ThAvHistogram_preaverage_kernel <<< auxVEC_cuReal3.device_size(mGPU), CUDATHREADS >>> 
				(cuaMesh.get_deviceobject(mGPU), cuaModules(mGPU), cuaNumModules(mGPU), (*pHa)(mGPU), auxVEC_cuReal3.get_deviceobject(mGPU), auxVEC_cuBReal.get_deviceobject(mGPU), num_av_cells, macrocell_dims);
		}
	}
	else {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			Get_ThAvHistogram_preaverage_kernel <<< auxVEC_cuReal3.device_size(mGPU), CUDATHREADS >>> 
				(cuaMesh.get_deviceobject(mGPU), cuaModules(mGPU), cuaNumModules(mGPU), Ha(mGPU), auxVEC_cuReal3.get_deviceobject(mGPU), auxVEC_cuBReal.get_deviceobject(mGPU), num_av_cells, macrocell_dims);
		}
	}
