...
MeshCUDA::MeshCUDA(Mesh* pMesh) :
	...
	cuMesh(mGPU),
	cuModules(mGPU), cuNumModules(mGPU),
	M(mGPU), M2(mGPU),
	Heff(mGPU), Heff2(mGPU)
{
	this->pMesh = pMesh;
...
Any MeshCUDA::GetAverageDisplayedMeshValue(Rect rel_rect, MESHDISPLAY_ quantity)
{
	...
	case MESHDISPLAY_MAGNETIZATION12:
	case MESHDISPLAY_MAGNETIZATION:
		return (DBL3)M.average_nonempty((cuRect)rel_rect);
		break;

	case MESHDISPLAY_MAGNETIZATION2:
		return (DBL3)M2.average_nonempty((cuRect)rel_rect);
		break;
	...
	case MESHDISPLAY_EFFECTIVEFIELD:
		if ((MOD_)pMesh->Get_Module_Heff_Display() == MOD_ALL || (MOD_)pMesh->Get_Module_Heff_Display() == MOD_ERROR) {
			...
			return (DBL3)Heff.average_nonempty((cuRect)rel_rect);
		}
		else {
			...
			if (pMesh->IsModuleSet(Module_Heff)) {
				...
				return (DBL3)pMesh->pMod(Module_Heff)->Get_Module_HeffCUDA().average_nonempty((cuRect)rel_rect);
			}
		}
		break;
	...
	case MESHDISPLAY_EFFECTIVEFIELD2:
		if ((MOD_)pMesh->Get_Module_Heff_Display() == MOD_ALL || (MOD_)pMesh->Get_Module_Heff_Display() == MOD_ERROR) {
			...
			return (DBL3)Heff2.average_nonempty((cuRect)rel_rect);
		}
		else {
			...
			if (pMesh->IsModuleSet(Module_Heff)) {
				...
				return (DBL3)pMesh->pMod(Module_Heff)->Get_Module_Heff2CUDA().average_nonempty((cuRect)rel_rect);
			}
		}
		break;
	...
	case MESHDISPLAY_ENERGY:
	{
		...
		if (pMesh->IsModuleSet(Module_Energy)) {
			...
			return (double)pMesh->pMod(Module_Energy)->Get_Module_EnergyCUDA().average_nonempty(rel_rect);
		}
	}
	break;
	...
	case MESHDISPLAY_ENERGY2:
	{
		...
		if (pMesh->IsModuleSet(Module_Energy)) {
			...
			return (double)pMesh->pMod(Module_Energy)->Get_Module_Energy2CUDA().average_nonempty(rel_rect);
		}
	}
	break;
	...
	case MESHDISPLAY_UDISP:
		return (DBL3)u_disp.average_nonempty((cuRect)rel_rect);
		break;

	case MESHDISPLAY_STRAINDIAG:
		return (DBL3)strain_diag.average_nonempty((cuRect)rel_rect);
		break;

	case MESHDISPLAY_STRAINODIAG:
		return (DBL3)strain_odiag.average_nonempty((cuRect)rel_rect);
		break;
...
void MeshCUDA::copy_auxVEC_cuBReal(VEC<double>& displayVEC)
{
...
bool MeshCUDA::Get_Kernel_Initialize_on_GPU(void)
{
	return pMesh->pSMesh->Get_Kernel_Initialize_on_GPU();
}
...
BError MeshCUDA::copy_shapes_from_cpu(void)
{
	...
	if (M.n.dim()) success &= M.set_from_cpuvec(pMesh->M);
...
BError MeshCUDA::copy_shapes_to_cpu(void)
{
	...
	//1a. shape magnetization
	if (M.n.dim()) success &= M.set_cpuvec(pMesh->M);
...
mcu_obj<ManagedDiffEq_CommonCUDA, ManagedDiffEqPolicy_CommonCUDA>& MeshCUDA::Get_ManagedDiffEq_CommonCUDA(void)
{ 
	return pMesh->pSMesh->Get_ManagedDiffEq_CommonCUDA(); 
}

mcu_obj<ManagedDiffEqFMCUDA, ManagedDiffEqPolicyFMCUDA>& MeshCUDA::Get_ManagedDiffEqFMCUDA(void)
{
