...
enum BWARNING_
{
	...
	BWARNING_CUDA_NOTP2P,					//multi-GPU configuration : devices not all in P2P mode
	BWARNING_ENUMSIZE
};
	...
	pOwner(pOwner_)
{
	...
	warnings[BWARNING_CUDA_NOTP2P] = std::string("Selected GPUs are not all in P2P mode (if using Windows, must enable SLI). This means direct memory transfers and UVA addressing not possible. Expect poor multi-GPU performance scaling. Multi-mesh simulations are disabled.");
