...
BError TransportCUDA_Spin_V_CMBND_Pri::set_pointers(MeshCUDA* pMeshCUDA, TransportBaseCUDA* pTransportBaseCUDA, int idx_device)
{
...
BError TransportCUDA_Spin_V_CMBND_Pri::set_pointers(Atom_MeshCUDA* paMeshCUDA, TransportBaseCUDA* pTransportBaseCUDA, int idx_device)
{
	BError error(__FUNCTION__);
...
__host__ void TransportCUDA_Spin_V_CMBND_Sec::construct_cu_obj(void)
{
	nullgpuptr(ppcuMesh);
...
__host__ void TransportCUDA_Spin_V_CMBND_Sec::destruct_cu_obj(void)
{
...
void TransportCUDA_Spin_V_CMBND_Sec::set_number_of_devices_mesh(int num_devices_cpu)
{
...
void TransportCUDA_Spin_V_CMBND_Sec::set_number_of_devices_amesh(int num_devices_cpu)
{
	if (!get_gpu_value(num_devices) && num_devices_cpu > 0) {
...
BError TransportCUDA_Spin_V_CMBND_Sec::set_pointers(MeshCUDA* pMeshCUDA, TransportBaseCUDA* pTransportBaseCUDA, int idx_device)
{
	BError error(__FUNCTION__);

	set_gpu_value(curr_device, idx_device);

	//TransportCUDA_Spin_V_CMBND_Sec objects are held in a mcu_obj, one for each device
	//However, each TransportCUDA_Spin_V_CMBND_Sec must hold pointers for all devices to allow access through UVA - thus loop here through allk devices and set pointers
...
BError TransportCUDA_Spin_V_CMBND_Sec::set_pointers(Atom_MeshCUDA* paMeshCUDA, TransportBaseCUDA* pTransportBaseCUDA, int idx_device)
{
	BError error(__FUNCTION__);
