...
void Atom_Mesh_BCC::Iterate_MonteCarlo(double acceptance_rate)
{
...
#if COMPILECUDA == 1
//Take a Monte Carlo step in this atomistic mesh
void Atom_Mesh_BCC::Iterate_MonteCarloCUDA(double acceptance_rate)
{ 
	if (mc_disabled) return;

	if (paMeshCUDA && mc_parallel) {

		if (mc_constrain) mc_acceptance_rate = reinterpret_cast<Atom_Mesh_BCCCUDA*>(paMeshCUDA)->Iterate_MonteCarloCUDA_Constrained(mc_cone_angledeg, acceptance_rate);
		else mc_acceptance_rate = reinterpret_cast<Atom_Mesh_BCCCUDA*>(paMeshCUDA)->Iterate_MonteCarloCUDA_Classic(mc_cone_angledeg, acceptance_rate);
	}
...
void Atom_Mesh_BCC::Iterate_MonteCarlo_Serial_Classic(void)
{
	//Serial version only implemented for simple cubic
}

void Atom_Mesh_BCC::Iterate_MonteCarlo_Serial_Constrained(void)
{
...
void Atom_Mesh_BCC::Iterate_MonteCarlo_Parallel_Classic(void)
{
	//number of moves in this step : one per each spin
	int num_moves = M1.get_nonempty_cells();
...
void Atom_Mesh_BCC::Iterate_MonteCarlo_Parallel_Constrained(void)
{
	//number of moves in this step : one per each spin
	int num_moves = M1.get_nonempty_cells();
