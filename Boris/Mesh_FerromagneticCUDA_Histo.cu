...
cuReal3 FMeshCUDA::GetThermodynamicAverageMagnetization(cuRect rectangle)
{
	...
	if (pHa) {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			GetThermodynamicAverageMagnetization_kernel <<< (M.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
				(cuMesh.get_deviceobject(mGPU), cuModules(mGPU), cuNumModules(mGPU), (*pHa)(mGPU), rectangle, aux_real(mGPU), aux_real3(mGPU));
		}
	}
	//No field (or rather ZeemanCUDA module not added)
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			GetThermodynamicAverageMagnetization_kernel <<< (M.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
				(cuMesh.get_deviceobject(mGPU), cuModules(mGPU), cuNumModules(mGPU), Ha(mGPU), rectangle, aux_real(mGPU), aux_real3(mGPU));
		}
	}
	...
	if (pHa) {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			Get_ThAvHistogram_FM_preaverage_kernel <<< auxVEC_cuReal3.device_size(mGPU), CUDATHREADS >>>
				(cuMesh.get_deviceobject(mGPU), cuModules(mGPU), cuNumModules(mGPU), (*pHa)(mGPU), auxVEC_cuReal3.get_deviceobject(mGPU), auxVEC_cuBReal.get_deviceobject(mGPU), num_av_cells, macrocell_dims);
		}
	}
	else {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			Get_ThAvHistogram_FM_preaverage_kernel <<< auxVEC_cuReal3.device_size(mGPU), CUDATHREADS >>>
				(cuMesh.get_deviceobject(mGPU), cuModules(mGPU), cuNumModules(mGPU), Ha(mGPU), auxVEC_cuReal3.get_deviceobject(mGPU), auxVEC_cuBReal.get_deviceobject(mGPU), num_av_cells, macrocell_dims);
		}
	}
