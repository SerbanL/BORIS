...
cuBReal Atom_Mesh_CubicCUDA::Iterate_MonteCarloCUDA_Classic(cuBReal mc_cone_angledeg, double target_acceptance_rate)
{
	if (mc_acceptance_reduction_counter == 0) {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			Zero_MCAux_Atom_Mesh_CubicCUDA <<< 1, CUDATHREADS >>> (mc_acceptance_rate(mGPU), cmc_M(mGPU));
		}
	}
		...
		if (mc_acceptance_reduction_counter == 0) {
			...
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				...
				Iterate_MonteCarloCUDA_Classic_Cubic_red_kernel <<< (M1.device_size(mGPU) / 2 + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
					(cuaMesh.get_deviceobject(mGPU), cuaModules(mGPU), cuaNumModules(mGPU), (*pHa)(mGPU), prng.get_deviceobject(mGPU), mc_cone_angledeg, mc_acceptance_rate(mGPU));
			}
			...
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				...
				Iterate_MonteCarloCUDA_Classic_Cubic_red_kernel <<< (M1.device_size(mGPU) / 2 + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
					(cuaMesh.get_deviceobject(mGPU), cuaModules(mGPU), cuaNumModules(mGPU), (*pHa)(mGPU), prng.get_deviceobject(mGPU), mc_cone_angledeg);
			}
	...
	//No field (or rather Atom_ZeemanCUDA module not added)
		...
		if (mc_acceptance_reduction_counter == 0) {
			...
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				...
				//with acceptance rate reduction
				Iterate_MonteCarloCUDA_Classic_Cubic_red_kernel <<< (M1.device_size(mGPU) / 2 + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
					(cuaMesh.get_deviceobject(mGPU), cuaModules(mGPU), cuaNumModules(mGPU), Ha(mGPU), prng.get_deviceobject(mGPU), mc_cone_angledeg, mc_acceptance_rate(mGPU));
			}
			...
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				...
				//without acceptance rate reduction
				Iterate_MonteCarloCUDA_Classic_Cubic_red_kernel <<< (M1.device_size(mGPU) / 2 + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
					(cuaMesh.get_deviceobject(mGPU), cuaModules(mGPU), cuaNumModules(mGPU), Ha(mGPU), prng.get_deviceobject(mGPU), mc_cone_angledeg);
			}
		...
		if (mc_acceptance_reduction_counter == 0) {
			...
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				...
				Iterate_MonteCarloCUDA_Constrained_Cubic_redblack_kernel <<< (mc_indices_red.size(mGPU) / 2 + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
					...
					cuaModules(mGPU), cuaNumModules(mGPU),
					(*pHa)(mGPU), prng.get_deviceobject(mGPU),
					cmc_M(mGPU), cmc_n(mGPU),
					mc_cone_angledeg, mc_acceptance_rate(mGPU));
			}
		...
		if (mc_acceptance_reduction_counter == 0) {
			...
			for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
				...
				Iterate_MonteCarloCUDA_Constrained_Cubic_redblack_kernel <<< (mc_indices_red.size(mGPU) / 2 + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
					...
					mc_indices_red.size(mGPU), mc_indices_red(mGPU),
					cuaModules(mGPU), cuaNumModules(mGPU),
					Ha(mGPU), prng.get_deviceobject(mGPU),
					cmc_M(mGPU), cmc_n(mGPU),
					mc_cone_angledeg, mc_acceptance_rate(mGPU));
			}
