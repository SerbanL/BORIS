...
#include "ManagedModulesPolicyCUDA.h"
...
protected:
	...
	mcu_VEC(cuReal3) Module_Heff, Module_Heff2;
	...
	//2nd VEC used for 2-sublattice modules
	mcu_VEC(cuBReal) Module_energy, Module_energy2;

	//Managed Module : just pass this into cuda kernels so you can access the above data through cuModule
	mcu_obj<ManagedModulesCUDA, ManagedModulesPolicyCUDA> cuModule;
...
protected:
	...
	cuReal3 CalculateTorque(mcu_VEC_VC(cuReal3)& M, cuRect& avRect);
...
public:
	...
	ModulesCUDA(void) :
		energy(mGPU), torque(mGPU), points_count(mGPU),
		Module_Heff(mGPU), Module_Heff2(mGPU),
		Module_energy(mGPU), Module_energy2(mGPU),
		cuModule(mGPU)
	{ 
		ZeroEnergy(); 
	...
	mcu_VEC(cuReal3)& Get_Module_Heff(void) { return Module_Heff; }
	mcu_VEC(cuReal3)& Get_Module_Heff2(void) { return Module_Heff2; }

	//Get VECs for display
	mcu_VEC(cuBReal)& Get_Module_Energy(void) { return Module_energy; }
	mcu_VEC(cuBReal)& Get_Module_Energy2(void) { return Module_energy2; }
	...
	cuBReal GetEnergyDensity(void) { return energy.to_cpu_sum(); }
