...
__device__ cuReal3 ManagedDiffEqAFMCUDA::SLLB(int idx, cuReal3& value_B)
{
	...
	cuReal3 mnormA = cu_normalize(M[idx]);
	cuReal3 mnormB = cu_normalize(M2[idx]);
	cuReal2 m = Mmag / Ms0;
	cuReal2 msq = m & m;
	...
	//sub-lattice B value so we can read it after
	value_B = (-(cuBReal)GAMMA * grel.j * msq.j / (msq.j + alpha.j * alpha.j)) * (M2[idx] ^ Heff2[idx]) + (-(cuBReal)GAMMA * grel.j * m.j * alpha.j / (msq.j + alpha.j * alpha.j)) * (mnormB ^ (M2[idx] ^ (Heff2[idx] + H_Thermal_Value_2))) +
		(cuBReal)GAMMA * grel.j * alpha_par.j * Ms0.j * (mnormB * (Heff2[idx] + Hl_2)) * mnormB + Torque_Thermal_Value_2;

	//return the sub-lattice A value as normal
	return (-(cuBReal)GAMMA * grel.i * msq.i / (msq.i + alpha.i * alpha.i)) * (M[idx] ^ Heff[idx]) + (-(cuBReal)GAMMA * grel.i * m.i * alpha.i / (msq.i + alpha.i * alpha.i)) * (mnormA ^ (M[idx] ^ (Heff[idx] + H_Thermal_Value))) +
		(cuBReal)GAMMA * grel.i * alpha_par.i * Ms0.i * (mnormA * (Heff[idx] + Hl_1)) * mnormA + Torque_Thermal_Value;
}
...
__device__ cuReal3 ManagedDiffEqAFMCUDA::SLLBSTT(int idx, cuReal3& value_B)
{
	...
	cuReal3 LLGSTT_Eval_A = (-(cuBReal)GAMMA * grel.i * msq.i / (msq.i + alpha.i * alpha.i)) * (M[idx] ^ Heff[idx]) + (-(cuBReal)GAMMA * grel.i * m.i * alpha.i / (msq.i + alpha.i * alpha.i)) * (mnormA ^ (M[idx] ^ (Heff[idx] + H_Thermal_Value))) +
		(cuBReal)GAMMA * grel.i * alpha_par.i * Ms0.i * (mnormA * (Heff[idx] + Hl_1)) * mnormA + Torque_Thermal_Value;

	cuReal3 LLGSTT_Eval_B = (-(cuBReal)GAMMA * grel.j * msq.j / (msq.j + alpha.j * alpha.j)) * (M2[idx] ^ Heff2[idx]) + (-(cuBReal)GAMMA * grel.j * m.j * alpha.j / (msq.j + alpha.j * alpha.j)) * (mnormB ^ (M2[idx] ^ (Heff2[idx] + H_Thermal_Value_2))) +
		(cuBReal)GAMMA * grel.j * alpha_par.j * Ms0.j * (mnormB * (Heff2[idx] + Hl_2)) * mnormB + Torque_Thermal_Value_2;
	...
	if (E.linear_size()) {
		...
		LLGSTT_Eval_A +=
			...
			((beta - alpha_perp_red.i) * (mnormA ^ u_dot_del_M_A)) -
				(alpha_perp_red.i * (beta - alpha_perp_red.i) * mnormA * (mnormA * u_dot_del_M_A))) * msq.i / (msq.i + alpha.i * alpha.i);

		LLGSTT_Eval_B +=
			(((1 + alpha_perp_red.j * beta) * u_dot_del_M_B) -
			((beta - alpha_perp_red.j) * (mnormB ^ u_dot_del_M_B)) -
				(alpha_perp_red.j * (beta - alpha_perp_red.j) * mnormB * (mnormB * u_dot_del_M_B))) * msq.j / (msq.j + alpha.j * alpha.j);
	}
