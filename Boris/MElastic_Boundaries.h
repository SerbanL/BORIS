...
public:
	...
	//get external force (Fx, Fy, Fz) at edge centers and vertex, where edges meet at given vertex index
	//off-diagonal components obtained at edge centres, and diagonal component at vertex
	//x normal face:
	//sxx = +/- Fx, sxy = +/- Fy, sxz = +/- Fz
	//y normal face
	//sxy = +/- Fx, syy = +/- Fy, syz = +/- Fz
	//z normal face
	//sxz = +/- Fx, syz = +/- Fy, szz = +/- Fz
	//In all off-diagonal cases the forces Fx, Fy, Fz are evaluated at centre of corresponding +x, +y, +z axis edges originating from given ijk_vertex
	DBL3 get_ext_force_edges(const INT3& ijk_vertex, const double& time)
	{
		if (use_constant_force) {
			...
			case -1:
				return DBL3(-Fext_constant.x, -Fext_constant.y, -Fext_constant.z);
				break;
			...
			case -1:
				x = (cells_box.e.j - ijk_vertex.j) * h_m.y;
				y = (ijk_vertex.k - cells_box.s.k) * h_m.z;
				return DBL3(-Fext_equation.evaluate_vector_x(x, y, time), -Fext_equation.evaluate_vector_y(x + h_m.y / 2, y, time), -Fext_equation.evaluate_vector_z(x, y + h_m.z / 2, time));
				break;
			...
			case -2:
				x = (ijk_vertex.i - cells_box.s.i) * h_m.x;
				y = (ijk_vertex.k - cells_box.s.k) * h_m.z;
				return DBL3(-Fext_equation.evaluate_vector_x(x + h_m.x / 2, y, time), -Fext_equation.evaluate_vector_y(x, y, time), -Fext_equation.evaluate_vector_z(x, y + h_m.z / 2, time));
				break;
			...
			case -3:
				x = (ijk_vertex.i - cells_box.s.i) * h_m.x;
				y = (cells_box.e.j - ijk_vertex.j) * h_m.y;
				return DBL3(-Fext_equation.evaluate_vector_x(x + h_m.x / 2, y, time), -Fext_equation.evaluate_vector_y(x, y + h_m.y / 2, time), -Fext_equation.evaluate_vector_z(x, y, time));
				break;
