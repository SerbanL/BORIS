...
void Atom_DemagMCUDA::set_Atom_DemagCUDA_pointers(void)
{
...
__device__ void Transfer_Moments_to_Magnetization_BlockReduction(
	cuVEC<cuReal3>& M, cuVEC_VC<cuReal3>& M1, cuReal3 *pshared_memory, unsigned BLOCKSIZE, unsigned int BLOCKOFFSET)
{
	...
	if (tidx == 0) {
		...
		if (BLOCKOFFSET == 0) M[ijk_m.i + ijk_m.j * n_m.x + ijk_m.k * n_m.x * n_m.y] = pshared_memory[0] * MUB / M.h.dim();
		...
		else M[ijk_m.i + ijk_m.j * n_m.x + ijk_m.k * n_m.x * n_m.y] += pshared_memory[0] * MUB / M.h.dim();
	}
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

//these will be compiled depending on which template N values are given in Atom_DemagMCUDA::Transfer_Moments_to_Magnetization, but do not exceed a maximum value (see below), and keep to powers of 2 
...
__global__ void Transfer_Moments_to_Magnetization_Kernel_BlockSize_N(
	cuVEC<cuReal3>& M, cuVEC_VC<cuReal3>& M1, unsigned int BLOCKOFFSET)
{
	__shared__ cuReal3 shared_memory[N];
	Transfer_Moments_to_Magnetization_BlockReduction(M, M1, shared_memory, N, BLOCKOFFSET);
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

//from M1 transfer to M, converting magnetic moments to magnetization
void Atom_DemagMCUDA::Transfer_Moments_to_Magnetization(void)
{
	...
	do {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			case 1:
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<1> << < M.device_size(mGPU), 1 >> >
					(M.get_deviceobject(mGPU), paMeshCUDA->M1.get_deviceobject(mGPU), BLOCKOFFSET);
				break;
			case 2:
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<2> << < M.device_size(mGPU), 2 >> >
					(M.get_deviceobject(mGPU), paMeshCUDA->M1.get_deviceobject(mGPU), BLOCKOFFSET);
				break;
			case 4:
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<4> << < M.device_size(mGPU), 4 >> >
					(M.get_deviceobject(mGPU), paMeshCUDA->M1.get_deviceobject(mGPU), BLOCKOFFSET);
				break;
			case 8:
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<8> << < M.device_size(mGPU), 8 >> >
					(M.get_deviceobject(mGPU), paMeshCUDA->M1.get_deviceobject(mGPU), BLOCKOFFSET);
				break;
			case 16:
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<16> << < M.device_size(mGPU), 16 >> >
					(M.get_deviceobject(mGPU), paMeshCUDA->M1.get_deviceobject(mGPU), BLOCKOFFSET);
				break;
			case 32:
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<32> << < M.device_size(mGPU), 32 >> >
					(M.get_deviceobject(mGPU), paMeshCUDA->M1.get_deviceobject(mGPU), BLOCKOFFSET);
				break;
			case 64:
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<64> << < M.device_size(mGPU), 64 >> >
					(M.get_deviceobject(mGPU), paMeshCUDA->M1.get_deviceobject(mGPU), BLOCKOFFSET);
				break;
			case 128:
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<128> << < M.device_size(mGPU), 128 >> >
					(M.get_deviceobject(mGPU), paMeshCUDA->M1.get_deviceobject(mGPU), BLOCKOFFSET);
				break;
			case 256:
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<256> << < M.device_size(mGPU), 256 >> >
					(M.get_deviceobject(mGPU), paMeshCUDA->M1.get_deviceobject(mGPU), BLOCKOFFSET);
				break;
			case 512:
				Transfer_Moments_to_Magnetization_Kernel_BlockSize_N<512> << < M.device_size(mGPU), 512 >> >
					(M.get_deviceobject(mGPU), paMeshCUDA->M1.get_deviceobject(mGPU), BLOCKOFFSET);
				break;
			}
		}
...
__global__ void Transfer_Demag_Field_Kernel(
...
void Atom_DemagMCUDA::Transfer_Demag_Field(mcu_VEC(cuReal3)& H)
{
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {

		Transfer_Demag_Field_Kernel <<< (paMeshCUDA->Heff1.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
...
void Atom_DemagMCUDA::Atom_Demag_EvalSpeedup_SubSelf(mcu_VEC(cuReal3)& H)
{
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {

		Atom_Demag_EvalSpeedup_SubSelf_Kernel <<< (H.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
			(H.get_deviceobject(mGPU), M.get_deviceobject(mGPU), selfDemagCoeff(mGPU));
	}
}

//QUINTIC
void Atom_DemagMCUDA::Atom_Demag_EvalSpeedup_SetExtrapField_AddSelf(mcu_VEC(cuReal3)& H, cuBReal a1, cuBReal a2, cuBReal a3, cuBReal a4, cuBReal a5, cuBReal a6)
{
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {

		Atom_Demag_EvalSpeedup_SetExtrapField_AddSelf_Kernel <<< (H.device_size(mGPU) + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
...
//QUARTIC
void Atom_DemagMCUDA::Atom_Demag_EvalSpeedup_SetExtrapField_AddSelf(mcu_VEC(cuReal3)& H, cuBReal a1, cuBReal a2, cuBReal a3, cuBReal a4, cuBReal a5)
{
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
...
//CUBIC
void Atom_DemagMCUDA::Atom_Demag_EvalSpeedup_SetExtrapField_AddSelf(mcu_VEC(cuReal3)& H, cuBReal a1, cuBReal a2, cuBReal a3, cuBReal a4)
{
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
...
//QUADRATIC
void Atom_DemagMCUDA::Atom_Demag_EvalSpeedup_SetExtrapField_AddSelf(mcu_VEC(cuReal3)& H, cuBReal a1, cuBReal a2, cuBReal a3)
{
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
...
//LINEAR
void Atom_DemagMCUDA::Atom_Demag_EvalSpeedup_SetExtrapField_AddSelf(mcu_VEC(cuReal3)& H, cuBReal a1, cuBReal a2)
{
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
...
//STEP
void Atom_DemagMCUDA::Atom_Demag_EvalSpeedup_SetExtrapField_AddSelf(mcu_VEC(cuReal3)& H)
{
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
