...
Atom_Mesh_BCC::Atom_Mesh_BCC(SuperMesh* pSMesh_) :
	Atom_Mesh(MESH_ATOM_BCC, pSMesh_, 2),
	ProgramStateNames(this,
		{
			...
			VINFO(pMod),
			VINFO(exclude_from_multiconvdemag), VINFO(dormant),
			VINFO(prng_seed),
			VINFO(trackWindow_velocity), VINFO(trackWindow_shift_clip), VINFO(trackWindow_shift_debt), VINFO(trackWindow_last_time), VINFO(idTrackShiftMesh),
	...
	meshODE(this)
{
	//Make M1 and Heff1 into BCC
	M1.set_number_of_sublattices({ DBL3(0.0), DBL3(0.5) });
	Heff1.set_number_of_sublattices({ DBL3(0.0), DBL3(0.5) });
}

Atom_Mesh_BCC::Atom_Mesh_BCC(Rect meshRect_, DBL3 h_, SuperMesh* pSMesh_) :
	Atom_Mesh(MESH_ATOM_BCC, pSMesh_, 2),
	ProgramStateNames(this,
		{
			//Mesh members
...
Atom_Mesh_BCC::~Atom_Mesh_BCC()
{
}

void Atom_Mesh_BCC::RepairObjectState(void)
{
...
BError Atom_Mesh_BCC::UpdateConfiguration(UPDATECONFIG_ cfgMessage)
{
	BError error(std::string(CLASS_STR(Atom_Mesh_BCC)) + "(" + (*pSMesh).key_from_meshId(meshId) + ")");
	...
	if (ucfg::check_cfgflags(cfgMessage, UPDATECONFIG_MESHCHANGE)) {
		...
		if (M1.linear_size()) {
			...
			if (!M1.assign(h, meshRect, DBL3(-mu_s, 0, 0))) return error(BERROR_OUTOFMEMORY_CRIT);
		}
...
void Atom_Mesh_BCC::UpdateConfiguration_Values(UPDATECONFIG_ cfgMessage)
{
	///////////////////////////////////////////////////////
...
BError Atom_Mesh_BCC::SwitchCUDAState(bool cudaState)
{
	BError error(std::string(CLASS_STR(Atom_Mesh_BCC)) + "(" + (*pSMesh).key_from_meshId(meshId) + ")");
	...
	if (cudaState) {
		...
		if (!pMeshBaseCUDA) {
			...
			pMeshBaseCUDA = new Atom_Mesh_BCCCUDA(this);
	...
	//SkyrmionTrack object also has CUDA objects it needs to construct/destruct as needed
	skyShift.SwitchCUDAState(cudaState);
	//same for dwPos
	dwPos.SwitchCUDAState(cudaState);
...
void Atom_Mesh_BCC::CoupleToDipoles(bool status)
{
#if COMPILECUDA == 1
...
double Atom_Mesh_BCC::CheckMoveMesh(void)
{
#if COMPILECUDA == 1
