...
BError SuperMesh::Set_Dormant(bool dormant, std::string meshName)
{
	BError error(__FUNCTION__);

	if (!contains(meshName)) return error(BERROR_INCORRECTNAME);

	pMesh[meshName]->Set_Dormant(dormant);
...
BError SuperMesh::Set_TAMR_Conductivity_Equation(std::string meshName, std::string equation_string)
{
	...
	pMesh[meshName]->CallModuleMethod(&TransportBase::Set_TAMR_Conductivity_Equation, equation_string);
