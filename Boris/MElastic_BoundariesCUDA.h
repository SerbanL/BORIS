...
public:
	...
	__device__ cuReal3 get_ext_force_edges(const cuINT3& ijk_vertex, const cuBReal& time)
	{
		if (use_constant_force) {
			...
			case -1:
				return cuReal3(-Fext_constant.x, -Fext_constant.y, -Fext_constant.z);
				break;
			...
			case -1:
				x = (cells_box.e.j - ijk_vertex.j) * h_m.y;
				y = (ijk_vertex.k - cells_box.s.k) * h_m.z;
				return cuReal3(-pFext_equation_x->evaluate(x, y, time), -pFext_equation_y->evaluate(x + h_m.y / 2, y, time), -pFext_equation_z->evaluate(x, y + h_m.z / 2, time));
				break;
			...
			case -2:
				x = (ijk_vertex.i - cells_box.s.i) * h_m.x;
				y = (ijk_vertex.k - cells_box.s.k) * h_m.z;
				return cuReal3(-pFext_equation_x->evaluate(x + h_m.x / 2, y, time), -pFext_equation_y->evaluate(x, y, time), -pFext_equation_z->evaluate(x, y + h_m.z / 2, time));
				break;
			...
			case -3:
				x = (ijk_vertex.i - cells_box.s.i) * h_m.x;
				y = (cells_box.e.j - ijk_vertex.j) * h_m.y;
				return cuReal3(-pFext_equation_x->evaluate(x + h_m.x / 2, y, time), -pFext_equation_y->evaluate(x, y + h_m.y / 2, time), -pFext_equation_z->evaluate(x, y, time));
				break;
