...
class SuperMesh : 
	...
	VEC<DBL3>, Rect, DBL3, DBL3, DBL3, DBL3, double,
...
private:
	...
	//globalField rectangle and cellsize stored separately
	//These are needed so globalField can be reconstructed fully when loaded from a bsm file, as ProgramState doesn't save them
	//Ideally globalField should have been defined as a VEC_VC, but it's too late now: making this change will break backwards compatibility
	//globalField_rect and globalField_h are simply used by RepairObjectState to remake globalField rect and h when loaded from bsm
	Rect globalField_rect;
	DBL3 globalField_h;

	//-----Algorithm data

	//global field shifting
	//during a simulation it's possible to shift globalField rectangle with fixed velocity (m/s)
	DBL3 globalField_velocity = DBL3();
	//when shifting due to non-zero velocity, if displacement is below the cliping distance (globalField_shift_clip), then add it to the shift_debt
	//when globalField_shift_debt exceeds clipping distance, then perform shift and reduce debt. This is to limit excessive shifting by small factors, since for non-zero velocity shifting is performed every iteration.
	DBL3 globalField_shift_clip = DBL3();
	DBL3 globalField_shift_debt = DBL3();
	double globalField_last_time = 0.0;
...
public:
	...
	{ 
		if (globalField.linear_size()) { globalField.rect = globalField_rect; globalField.h = globalField_h; }	//see comments for globalField definition
		error_on_create = UpdateConfiguration(UPDATECONFIG_REPAIROBJECTSTATE);
	}
	...
	//switch CUDA state on/off; when switching on set selected cuda device number(s)
	BError SwitchCUDAState(bool cudaState, std::vector<int> devices = {});
...
#if COMPILECUDA == 1
	mcu_obj<ManagedDiffEq_CommonCUDA, ManagedDiffEqPolicy_CommonCUDA>& Get_ManagedDiffEq_CommonCUDA(void) { return odeSolver.Get_pODECUDA()->Get_ManagedDiffEq_CommonCUDA(); }
	mcu_obj<ManagedAtom_DiffEq_CommonCUDA, ManagedAtom_DiffEqPolicy_CommonCUDA>& Get_ManagedAtom_DiffEq_CommonCUDA(void) { return atom_odeSolver.Get_pODECUDA()->Get_ManagedAtom_DiffEq_CommonCUDA(); }
#endif
	...
	BError GenerateUGrains2D(std::string meshName, double spacing, double variation, int seed);
	...
	BError GenerateUGrains3D(std::string meshName, double spacing, double variation, int seed);
	...
	mcu_VEC(cuReal3)& GetGlobalFieldCUDA(void) { return pSMeshCUDA->GetGlobalField(); }
#endif
