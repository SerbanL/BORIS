...
__global__ void CalculateDMExchangeCoupling_Atom_kernel(
	...
	cuBReal& energy, bool do_reduction)
{
	...
	if (box_idx < box_sizes.dim()) {
		...
		if (M1_pri.is_not_empty(cell1_idx) && M1_pri.is_cmbnd(cell1_idx)) {
			...
			Hexch_D = (D / (MUB_MU0 * mu_s)) * (cuReal3(0.0, -m_2.z, m_2.y) + cuReal3(0.0, -m_m1.z, m_m1.y));
...
// FM to Atomistic
__global__ void CalculateDMExchangeCoupling_FM_to_Atom_kernel(
...
void Atom_DMExchangeCUDA::CalculateExchangeCoupling(mcu_val<cuBReal>& energy)
{
		...
		if (pContactingMeshes[idx_pri]->is_atomistic() && pContactingMeshes[idx_sec]->is_atomistic()) {
			...
			if (pMeshBaseCUDA->CurrentTimeStepSolved()) {
				...
				for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
					...
					CalculateDMExchangeCoupling_Atom_kernel <<< (size + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
			...
			if (pMeshBaseCUDA->CurrentTimeStepSolved()) {
				...
				for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
					...
					CalculateDMExchangeCoupling_FM_to_Atom_kernel <<< (size + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
