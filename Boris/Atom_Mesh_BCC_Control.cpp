...
void Atom_Mesh_BCC::SetMagAngle(double polar, double azim, Rect rectangle)
{
#if COMPILECUDA == 1
	...
	if (paMeshCUDA) {
		...
		if (rectangle.IsNull()) {
			...
			paMeshCUDA->M1.setnonempty(Polar_to_Cartesian(vec_convert(mu_s.get_sublattices(), DBL3(1, 0, 0), DBL3(0, polar, azim))));
		}
		else {

			paMeshCUDA->M1.setrectnonempty((cuRect)rectangle, Polar_to_Cartesian(vec_convert(mu_s.get_sublattices(), DBL3(1, 0, 0), DBL3(0, polar, azim))));
		}
	...
	if (M1.linear_size()) {
		...
		if (rectangle.IsNull()) {
			...
			M1.setnonempty(Polar_to_Cartesian(vec_convert(mu_s.get_sublattices(), DBL3(1, 0, 0), DBL3(0, polar, azim))));
		}
		else {

			M1.setrectnonempty(rectangle, Polar_to_Cartesian(vec_convert(mu_s.get_sublattices(), DBL3(1, 0, 0), DBL3(0, polar, azim))));
		}
	}
}

//set magnetization angle only in given shape
void Atom_Mesh_BCC::SetMagAngle_Shape(double polar, double azim, std::vector<MeshShape> shapes)
{
#if COMPILECUDA == 1
	...
	if (M1.linear_size()) {
		//BCC
		//M1.shape_setvalue(shapes, Polar_to_Cartesian(vec_convert(mu_s.get_sublattices(), DBL3(1, 0, 0), DBL3(0, polar, azim))));
	}
...
void Atom_Mesh_BCC::SetMagAngle_Object(double polar, double azim, DBL3 position)
{
#if COMPILECUDA == 1
	...
	if (M1.linear_size()) {
		...
		M1.setobject(Polar_to_Cartesian(vec_convert(mu_s.get_sublattices(), DBL3(1, 0, 0), DBL3(0, polar, azim))), position);
	}
...
void Atom_Mesh_BCC::SetMagFlower(int direction, DBL3 centre, double radius, double thickness)
{
#if COMPILECUDA == 1
	//refresh M from gpu memory
	if (paMeshCUDA) paMeshCUDA->M1.copy_to_cpuvec(M1);
...
void Atom_Mesh_BCC::SetMagOnion(int direction, DBL3 centre, double radius1, double radius2, double thickness)
{
#if COMPILECUDA == 1
	//refresh M from gpu memory
	if (paMeshCUDA) paMeshCUDA->M1.copy_to_cpuvec(M1);
...
void Atom_Mesh_BCC::SetMagCrosstie(int direction, DBL3 centre, double radius, double thickness)
{
#if COMPILECUDA == 1
	//refresh M from gpu memory
	if (paMeshCUDA) paMeshCUDA->M1.copy_to_cpuvec(M1);
...
void Atom_Mesh_BCC::SetInvertedMag(bool x, bool y, bool z)
{
#if COMPILECUDA == 1
	//refresh M from gpu memory
	if (paMeshCUDA) paMeshCUDA->M1.copy_to_cpuvec(M1);
...
void Atom_Mesh_BCC::SetMirroredMag(std::string axis)
{
#if COMPILECUDA == 1
	//refresh M from gpu memory
	if (paMeshCUDA) paMeshCUDA->M1.copy_to_cpuvec(M1);
...
void Atom_Mesh_BCC::SetRandomMag(int seed)
{
#if COMPILECUDA == 1
	//refresh M from gpu memory
	if (paMeshCUDA) paMeshCUDA->M1.copy_to_cpuvec(M1);
...
void Atom_Mesh_BCC::SetRandomXYMag(int seed)
{
#if COMPILECUDA == 1
	//refresh M from gpu memory
	if (paMeshCUDA) paMeshCUDA->M1.copy_to_cpuvec(M1);
...
void Atom_Mesh_BCC::SetMagDomainWall(int longitudinal, int transverse, double width, double position)
{
#if COMPILECUDA == 1
	//refresh M from gpu memory
	if (paMeshCUDA) paMeshCUDA->M1.copy_to_cpuvec(M1);
...
void Atom_Mesh_BCC::SetSkyrmion(int orientation, int chirality, Rect skyrmion_rect)
{
#if COMPILECUDA == 1
	//refresh M from gpu memory
	if (paMeshCUDA) paMeshCUDA->M1.copy_to_cpuvec(M1);
...
void Atom_Mesh_BCC::SetSkyrmionBloch(int orientation, int chirality, Rect skyrmion_rect)
{
#if COMPILECUDA == 1
	//refresh M from gpu memory
	if (paMeshCUDA) paMeshCUDA->M1.copy_to_cpuvec(M1);
...
void Atom_Mesh_BCC::SetMagFromData(VEC<DBL3>& data, const Rect& dstRect)
{
#if COMPILECUDA == 1
	//refresh M from gpu memory
	if (paMeshCUDA) paMeshCUDA->M1.copy_to_cpuvec(M1);
