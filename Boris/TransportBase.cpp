...
void TransportBase::MoveMesh_Transport(double x_shift)
{
	...
	if (pMeshBase->pMeshBaseCUDA) {
		...
		pMeshBase->pMeshBaseCUDA->elC.shift_x(x_shift, shift_rect);

		//shift spin accumulation if present
		if (pMeshBase->S.linear_size()) pMeshBase->pMeshBaseCUDA->S.shift_x(x_shift, shift_rect);
...
DBL3 TransportBase::GetAverageChargeCurrent(Rect rectangle, std::vector<MeshShape> shapes)
{
	...
	if (pTransportBaseCUDA) {
		...
		return cuReal3(pTransportBaseCUDA->displayVEC_VC.average_nonempty(rectangle));
	}
#endif
...
DBL3 TransportBase::GetAverageSpinCurrent(int component, Rect rectangle, std::vector<MeshShape> shapes)
{
	...
	if (pTransportBaseCUDA) {
		...
		if (pSMesh->SolveSpinCurrent()) return cuReal3(pTransportBaseCUDA->displayVEC.average_nonempty(rectangle));
...
mcu_VEC_VC(cuReal3)& TransportBase::GetChargeCurrentCUDA(void)
{
	return pTransportBaseCUDA->GetChargeCurrent();
}

mcu_VEC(cuReal3)& TransportBase::GetSpinCurrentCUDA(int component)
{
	return pTransportBaseCUDA->GetSpinCurrent(component);
}

mcu_VEC(cuReal3)& TransportBase::GetSpinTorqueCUDA(void)
{
