...
DBL3 Atom_Mesh_BCC::Average_mxdmdt(Rect avRect)
{
	if (avRect.IsNull()) avRect = meshRect - meshRect.s;
	Box box = M1.box_from_rect_max(avRect + meshRect.s);

#if COMPILECUDA == 1
	if (paMeshCUDA) return reinterpret_cast<Atom_Mesh_BCCCUDA*>(paMeshCUDA)->Average_mxdmdt(box);
#endif
