...
__device__ bool from_notyRegion_in_xRegion_R_SDemag(
...
__global__ void Copy_M_Input_xRegion_SDemag_kernel(
	...
	cuSZ3& n, cuSZ3& N, cuINT2& xRegion_R, cuINT2& yRegion, int& num_devices)
{
	...
	if (from_notyRegion_in_xRegion_R_SDemag(idx, component_idx, idx_out, idx_in, n, N, xRegion_R, yRegion, num_devices))
		M_Input_xRegion[component_idx][idx_out] = M[idx_in];
}

template <typename VECType>
__global__ void Copy_M_Input_xRegion_SDemag_halfprecision_kernel(
...
void SDemagMCUDA_single::Copy_M_Input_xRegion(bool half_precision)
{
	if (!half_precision) {

		Copy_M_Input_xRegion_SDemag_kernel <<< (nxRegion_R * (n.y - nyRegion) * n.z + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
			(pSDemagMCUDA->sm_Vals.get_deviceobject(device_index), Real_xRegion_arr, cun, cuN, cuxRegion_R, cuyRegion, cunum_devices);
	}
	else {

		Copy_M_Input_xRegion_SDemag_halfprecision_kernel <<< (nxRegion_R * (n.y - nyRegion) * n.z + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
			(pSDemagMCUDA->sm_Vals.get_deviceobject(device_index), Real_xRegion_half_arr, cun, cuN, cuxRegion_R, cuyRegion, cunum_devices, normalization_M);
	}
}
