...
protected:
	...
	//a dormant mesh is skipped during simulation iterations (i.e. it's simply a data container and doesn't interact with other meshes in the usual way)
	//dormant meshes are useful for track shifting algorithm, as they can be paired with a "simulation window mesh"
	//dormant meshes are automatically excluded from multilayered demag convolution
	bool dormant = false;

	//--------Track Shifting Algorithm Data

	//data similar to global field shifting in SuperMesh, but used to shift a simulation window
	//if velocity set non-zero then this will be a dormant mesh, which holds data to be shifted within a smaller simulation window mesh
	DBL3 trackWindow_velocity = DBL3();
	DBL3 trackWindow_shift_clip = DBL3();
	DBL3 trackWindow_shift_debt = DBL3();
	double trackWindow_last_time = 0.0;

	//if this is a dormant "holder" mesh with track shifting algorithm enabled, this vector will be non-empty and hold meshId values to "simulation window meshes" for which track shifting must be done
	//NOTE: all simulation window meshes linked to this dormant mesh will be moved at the same velocity (it's actually the dormant mesh which moves in the opposite direction so the required data shift results)
	std::vector<int> idTrackShiftMesh;
...
public:
	...
	virtual void Shift_Mesh_Rectangle(DBL3 shift) = 0;
	...
	virtual BError GenerateUGrains2D(double spacing, double variation, int seed) = 0;
	...
	virtual BError GenerateUGrains3D(double spacing, double variation, int seed) = 0;
