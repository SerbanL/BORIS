...
public:
	...
	//A-Function for N
	__device__ cuReal3 A_func_sec(
		int cell1_idx, int cell2_idx, cuReal3 relpos_m1, cuReal3 shift, cuReal3 stencil,
		TransportCUDA_Spin_S_CMBND_Sec& trans_sec, TransportCUDA_Spin_S_CMBND_Pri& trans_pri) const
	{
		int device = 0;
		...
		cuVEC_VC<cuBReal>& V_sec = (trans_sec.ppcuMesh ? *(trans_sec.ppcuMesh[device]->pV) : *(trans_sec.ppcuaMesh[device]->pV));
		...
		if (primary_top) {
			...
			if (trans_sec.ppcuMesh) {
				...
				Gmix = *(trans_sec.ppcuMesh[device]->pGmix);
				trans_sec.ppcuMesh[device]->update_parameters_atposition(relpos_m1, *(trans_sec.ppcuMesh[device]->pGi), Gi, *(trans_sec.ppcuMesh[device]->pGmix), Gmix);
			}
			else {

				Gi = *(trans_sec.ppcuaMesh[device]->pGi);
				Gmix = *(trans_sec.ppcuaMesh[device]->pGmix);
				trans_sec.ppcuaMesh[device]->update_parameters_atposition(relpos_m1, *(trans_sec.ppcuaMesh[device]->pGi), Gi, *(trans_sec.ppcuaMesh[device]->pGmix), Gmix);
			}
		}
		...
		if (trans_pri.stsolve == STSOLVE_FERROMAGNETIC || trans_pri.stsolve == STSOLVE_FERROMAGNETIC_ATOM) {
			...
			if (cuIsNZ((cuBReal)pump_eff_pri) && trans_pri.pspin_V_cmbnd_funcs_pri->pdM_dt) {

				cuReal3 dmdt = 1.5 * cu_normalize((*(trans_pri.pspin_V_cmbnd_funcs_pri->pdM_dt))[idx1_M], M_pri[idx1_M]) - 0.5 * cu_normalize((*(trans_pri.pspin_V_cmbnd_funcs_pri->pdM_dt))[idx2_M], M_pri[idx2_M]);
		...
		cuBReal V_sec_val = 1.5 * V_sec.weighted_average(devrelpos_m1, stencil) - 0.5 * V_sec.weighted_average(devrelpos_m1 + shift, stencil);
	...
	//B-Function for N
	__device__ cuReal33 B_func_sec(
		int cell1_idx, int cell2_idx, cuReal3 relpos_m1, cuReal3 shift, cuReal3 stencil,
		TransportCUDA_Spin_S_CMBND_Sec& trans_sec, TransportCUDA_Spin_S_CMBND_Pri& trans_pri) const
	{
		int device = 0;
	...
	//B-Function for F
	__device__ cuReal33 B_func_pri(
		int cell1_idx, int cell2_idx, cuReal3 relpos_m1, cuReal3 shift, cuReal3 stencil,
		TransportCUDA_Spin_S_CMBND_Sec& trans_sec, TransportCUDA_Spin_S_CMBND_Pri& trans_pri) const
	{
		int device = 0;
		trans_sec.global_relpos_to_device_relpos(relpos_m1, device);
...
public:
		...
		TransportCUDA_Spin_S_CMBND_Sec& trans_sec, TransportCUDA_Spin_S_CMBND_Pri& trans_pri) const
	{
		...
		cuBReal pump_eff_sec = (trans_sec.ppcuMesh ? *(trans_sec.ppcuMesh[device]->ppump_eff) : *(trans_sec.ppcuaMesh[device]->ppump_eff));
		...
		if (trans_sec.stsolve == STSOLVE_FERROMAGNETIC || trans_sec.stsolve == STSOLVE_FERROMAGNETIC_ATOM) {
			...
			if (trans_sec.ppcuMesh) trans_sec.ppcuMesh[device]->update_parameters_atposition(relpos_m1, *(trans_sec.ppcuMesh[device]->ppump_eff), pump_eff_sec);
			else trans_sec.ppcuaMesh[device]->update_parameters_atposition(relpos_m1, *(trans_sec.ppcuaMesh[device]->ppump_eff), pump_eff_sec);

			//M value at the interface obtained using interpolation
			m = 1.5 * cu_normalize(M_sec.weighted_average(devrelpos_m1, stencil)) - 0.5 * cu_normalize(M_sec.weighted_average(devrelpos_m1 + shift, stencil));

			if (cuIsNZ((cuBReal)pump_eff_sec) && trans_sec.ppspin_V_cmbnd_funcs_pri[device]->pdM_dt) {

				int idx1_M = M_sec.position_to_cellidx(devrelpos_m1);
				int idx2_M = M_sec.position_to_cellidx(devrelpos_m1 + shift);

				cuReal3 dmdt = 1.5 * cu_normalize((*(trans_sec.ppspin_V_cmbnd_funcs_pri[device]->pdM_dt))[idx1_M], M_sec[idx1_M]) - 0.5 * cu_normalize((*(trans_sec.ppspin_V_cmbnd_funcs_pri[device]->pdM_dt))[idx2_M], M_sec[idx2_M]);
