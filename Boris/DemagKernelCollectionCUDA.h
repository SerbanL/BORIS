...
private:
	...
	std::vector<bool> zshifted, xshifted;
...
private:
	...
	BError Calculate_Demag_Kernels_2D_xShifted(int index, bool initialize_on_gpu);
	BError Calculate_Demag_Kernels_2D_xShifted_onGPU(int index);
	...
	BError Calculate_Demag_Kernels_3D_xShifted(int index, bool initialize_on_gpu);
	BError Calculate_Demag_Kernels_3D_xShifted_onGPU(int index);
	...
	//for kernel to be considered already computed, it must be on same device (device_index)
	std::shared_ptr<cu_obj<cuKerType>> KernelAlreadyComputed(cuReal3 shift, cuReal3 h_src, cuReal3 h_dst, int device_index);
