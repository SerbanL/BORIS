...
TMRCUDA::TMRCUDA(TMR* pTMR_) : 
	...
	RAtmr_p_equation(mGPU), RAtmr_ap_equation(mGPU)
{
	pTMR = pTMR_;
	pMesh = pTMR->pMesh;
	...
	if (!error_on_create) {
		...
		error_on_create = spin_V_cmbnd_funcs_pri.set_pointers(pMeshCUDA, this);
		error_on_create = spin_V_cmbnd_funcs_sec.set_pointers(pMeshCUDA, this);

		error_on_create = spin_S_cmbnd_funcs_pri.set_pointers(pMeshCUDA, this);
		error_on_create = spin_S_cmbnd_funcs_sec.set_pointers(pMeshCUDA, this);
	}
}
	...
	if (!error) {
		...
		for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
			...
			for (int idx = 0; idx < pTMR->pMeshFM_Bot.size(); idx++) {
				...
				pMeshFM_Bot.push_back(mGPU, pTMR->pMeshFM_Bot[idx]->pMeshCUDA->cuMesh.get_managed_object(mGPU));
			}

			for (int idx = 0; idx < pTMR->pMeshFM_Top.size(); idx++) {

				pMeshFM_Top.push_back(mGPU, pTMR->pMeshFM_Top[idx]->pMeshCUDA->cuMesh.get_managed_object(mGPU));
			}

			for (int idx = 0; idx < pTMR->pMeshAtom_Bot.size(); idx++) {

				pMeshAtom_Bot.push_back(mGPU, pTMR->pMeshAtom_Bot[idx]->paMeshCUDA->cuaMesh.get_managed_object(mGPU));
			}

			for (int idx = 0; idx < pTMR->pMeshAtom_Top.size(); idx++) {

				pMeshAtom_Top.push_back(mGPU, pTMR->pMeshAtom_Top[idx]->paMeshCUDA->cuaMesh.get_managed_object(mGPU));
			}
		}
...
BError TMRCUDA::UpdateConfiguration(UPDATECONFIG_ cfgMessage)
{
	...
	if (ucfg::check_cfgflags(cfgMessage, UPDATECONFIG_MESHSHAPECHANGE, UPDATECONFIG_MESHCHANGE)) {
		...
		if (pMeshCUDA->V.size_cpu().dim()) {
			...
			success &= pMeshCUDA->V.resize(pMeshCUDA->h_e, pMeshCUDA->meshRect, pMeshCUDA->elC);
		}
		else {

			success &= pMeshCUDA->V.assign(pMeshCUDA->h_e, pMeshCUDA->meshRect, (cuBReal)0.0, pMeshCUDA->elC);
		}
		...
		if (pMeshCUDA->E.size_cpu().dim()) {
			...
			success &= pMeshCUDA->E.resize(pMeshCUDA->h_e, pMeshCUDA->meshRect, pMeshCUDA->elC);
		}
		else {

			success &= pMeshCUDA->E.assign(pMeshCUDA->h_e, pMeshCUDA->meshRect, cuReal3(0.0), pMeshCUDA->elC);
		}
	}
	...
	if (ucfg::check_cfgflags(cfgMessage, UPDATECONFIG_MESHSHAPECHANGE, UPDATECONFIG_MESHCHANGE, UPDATECONFIG_ODE_SOLVER)) {
		...
		if (success && stsolve != STSOLVE_NONE) {
			...
			if (pMeshCUDA->S.size_cpu().dim()) {
				...
				success &= pMeshCUDA->S.resize(pMeshCUDA->h_e, pMeshCUDA->meshRect, pMeshCUDA->elC);
			}
			else {

				success &= pMeshCUDA->S.assign(pMeshCUDA->h_e, pMeshCUDA->meshRect, cuReal3(0.0), pMeshCUDA->elC);
			}
		}
		else {
...
{
	cuBReal conductance = pMeshCUDA->elC.sum_nonempty(rect) * pMeshCUDA->h_e.x * pMeshCUDA->h_e.y / (pMeshCUDA->meshRect.height() * pMeshCUDA->n_e.k);
