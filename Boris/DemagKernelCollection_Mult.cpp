...
//x shifted version
void DemagKernelCollection::KernelMultiplication_2D_xShifted(VEC<ReIm3>& In, VEC<ReIm3>& Out, VEC<ReIm3>& Kdiag, VEC<ReIm3>& Kodiag)
{
	//Full multiplication with use of kernel symmetries, z-shifted version
...
#pragma omp parallel for
	for (int j = 1; j < N.y / 2; j++) {
		for (int i = 0; i < (N.x / 2 + 1); i++) {
			...
			Out[idx_h].x += (Kdiag[idx_l].x * FM_h.x) - (Kodiag[idx_l].x * FM_h.y) + (Kodiag[idx_l].y * FM_h.z);
			Out[idx_h].y += -1 * (Kodiag[idx_l].x * FM_h.x) + (Kdiag[idx_l].y * FM_h.y) - (Kodiag[idx_l].z * FM_h.z);
			Out[idx_h].z += (Kodiag[idx_l].y * FM_h.x) - (Kodiag[idx_l].z * FM_h.y) + (Kdiag[idx_l].z * FM_h.z);
		}
	}
...
void DemagKernelCollection::KernelMultiplication_3D_xShifted(VEC<ReIm3>& In, VEC<ReIm3>& Out, VEC<ReIm3>& Kdiag, VEC<ReIm3>& Kodiag)
{
...
void DemagKernelCollection::KernelMultiplication_2D(std::vector<VEC<ReIm3>*>& Incol, VEC<ReIm3>& Out)
{
	...
	for (int mesh_index = 0; mesh_index < Incol.size(); mesh_index++) {
		...
		else if (kernels[mesh_index]->xshifted) {

			KernelMultiplication_2D_xShifted(*Incol[mesh_index], Out, kernels[mesh_index]->Kdiag_cmpl, kernels[mesh_index]->Kodiag_cmpl);
		}
...
void DemagKernelCollection::KernelMultiplication_3D(std::vector<VEC<ReIm3>*>& Incol, VEC<ReIm3>& Out)
{
	...
	for (int mesh_index = 0; mesh_index < Incol.size(); mesh_index++) {
		...
		//x-shifted : use symmetries
			...
			KernelMultiplication_3D_xShifted(*Incol[mesh_index], Out, kernels[mesh_index]->Kdiag_cmpl, kernels[mesh_index]->Kodiag_cmpl);
		}
