...
DemagMCUDA_single::DemagMCUDA_single(MeshCUDA* pMeshCUDA_, DemagMCUDA *pDemagMCUDA_, int device_index_) :
	ConvolutionCUDA<DemagMCUDA_single, DemagKernelCUDA>(),
	device_index(device_index_)
{
	pMeshCUDA = pMeshCUDA_;
	pDemagMCUDA = pDemagMCUDA_;
...
DemagMCUDA_single::~DemagMCUDA_single()
{
	for (int idx = 0; idx < mGPU.get_num_devices(); idx++) {
...
BError DemagMCUDA_single::Initialize(void)
{
	BError error(CLASS_STR(DemagMCUDA_single));
		...
		for (int idx = 0; idx < mGPU.get_num_devices(); idx++) {
			...
			if (mGPU.get_num_devices() > 1) {	
				...
				if (idx != device_index) {
					...
					int region_xdim = pDemagMCUDA->pDemagMCUDA[idx]->nxRegion;
					int region_xdim_R = pDemagMCUDA->pDemagMCUDA[idx]->nxRegion_R;
					int region_ydim = pDemagMCUDA->pDemagMCUDA[idx]->nyRegion;

					if (!mGPU.get_halfprecision_transfer()) {

						if (success) success = Real_xRegion[idx]->resize(nxRegion_R * region_ydim * pMeshCUDA->n.z);
						if (success) success = Real_yRegion[idx]->resize(region_xdim_R * nyRegion * pMeshCUDA->n.z);

						if (success) success = Complex_xRegion[idx]->resize(nxRegion * region_ydim * pMeshCUDA->n.z * 3);
						if (success) success = Complex_yRegion[idx]->resize(region_xdim * nyRegion * pMeshCUDA->n.z * 3);
					}
					else {

						if (success) success = Real_xRegion_half[idx]->resize(nxRegion_R * region_ydim * pMeshCUDA->n.z * 3);
						if (success) success = Real_yRegion_half[idx]->resize(region_xdim_R * nyRegion * pMeshCUDA->n.z * 3);

						//x2 since packing complex into reals
						if (success) success = Complex_xRegion_half[idx]->resize(nxRegion * region_ydim * pMeshCUDA->n.z * 3 * 2);
						//x2 since packing complex into reals
						if (success) success = Complex_yRegion_half[idx]->resize(region_xdim * nyRegion * pMeshCUDA->n.z * 3 * 2);
					}
				}
	...
	normalization.from_cpu(pMeshCUDA->Ms(device_index)->get0_cpu() * N.dim());
	normalization_M.from_cpu(pMeshCUDA->Ms(device_index)->get0_cpu());

	return error;
}

BError DemagMCUDA_single::UpdateConfiguration(UPDATECONFIG_ cfgMessage)
{
	BError error(CLASS_STR(DemagMCUDA_single));
...
void DemagMCUDA_single::UpdateField(void)
{
}
