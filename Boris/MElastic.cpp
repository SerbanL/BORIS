...
MElastic::MElastic(Mesh *pMesh_) :
	...
	VINFO(vx2), VINFO(vy2), VINFO(vz2), VINFO(sdd2), VINFO(sxy2), VINFO(sxz2), VINFO(syz2),
...
BError MElastic::Initialize(void)
{
	...
	if (!initialized && pSMEl->el_dT > 0.0) {
		...
		//Must have at least one fixed surface defined (unless solver iteration disabled)
		if (!pSMEl->fixed_u_surfaces.size() && pSMEl->el_dT > MINTIMESTEP) return error(BERROR_INCORRECTCONFIG);
...
BError MElastic::UpdateConfiguration(UPDATECONFIG_ cfgMessage)
{
	...
	if (ucfg::check_cfgflags(cfgMessage, UPDATECONFIG_MELASTIC, UPDATECONFIG_MESHSHAPECHANGE, UPDATECONFIG_MESHCHANGE)) {
		...
		if (crystal == CRYSTAL_TRIGONAL) {

			success &= vx2.resize(SZ3(pMesh->n_m.x, pMesh->n_m.y, pMesh->n_m.z));
			success &= vy2.resize(SZ3(pMesh->n_m.x + 1, pMesh->n_m.y + 1, pMesh->n_m.z));
			success &= vz2.resize(SZ3(pMesh->n_m.x + 1, pMesh->n_m.y, pMesh->n_m.z + 1));

			success &= sdd2.resize(SZ3(pMesh->n_m.x + 1, pMesh->n_m.y, pMesh->n_m.z));
			success &= sxy2.resize(SZ3(pMesh->n_m.x, pMesh->n_m.y + 1, pMesh->n_m.z));
			success &= sxz2.resize(SZ3(pMesh->n_m.x, pMesh->n_m.y, pMesh->n_m.z + 1));
			success &= syz2.resize(SZ3(pMesh->n_m.x + 1, pMesh->n_m.y + 1, pMesh->n_m.z + 1));
		}
		else {
