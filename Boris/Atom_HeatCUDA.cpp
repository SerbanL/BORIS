	...
	HeatBaseCUDA()
{
	...
	if (!error_on_create) {
		...
		error_on_create = temp_cmbnd_funcs_pri.set_pointers(paMeshCUDA);
		error_on_create = temp_cmbnd_funcs_sec.set_pointers(paMeshCUDA);
	}
}
...
BError Atom_HeatCUDA::UpdateConfiguration(UPDATECONFIG_ cfgMessage)
{
	...
	if (ucfg::check_cfgflags(cfgMessage, UPDATECONFIG_MESHSHAPECHANGE, UPDATECONFIG_MESHCHANGE, UPDATECONFIG_MODULEADDED, UPDATECONFIG_MODULEDELETED, UPDATECONFIG_HEAT_MODELTYPE)) {
		...
		if (paMeshCUDA->M1.size_cpu().dim()) {
			...
			if (success) {
				...
				if (paMeshCUDA->Temp.size_cpu().dim()) {
					...
					success = paMeshCUDA->Temp.resize(paMesh->h_t, paMesh->meshRect, paMeshCUDA->elC);

					//lattice temperature for many-temperature models
					if (paHeat->tmtype == TMTYPE_2TM) success &= paMeshCUDA->Temp_l.resize(paMesh->h_t, paMesh->meshRect, paMeshCUDA->Temp);
				}
				else {
		...
		if (success) success = heatEq_RHS.resize(paMesh->h_t, paMesh->meshRect);
