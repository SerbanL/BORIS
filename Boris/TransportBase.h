...
#if COMPILECUDA == 1
	mcu_VEC_VC(cuReal3)& GetChargeCurrentCUDA(void);
	mcu_VEC(cuReal3)& GetSpinCurrentCUDA(int component);
	mcu_VEC(cuReal3)& GetSpinTorqueCUDA(void);
#endif
...
public:
	...
	BError Set_TAMR_Conductivity_Equation(std::string equation_string) { return BError(); }
