...
__device__ void ManagedMeshCUDA::update_parameters_mcoarse_spatial(int mcell_idx, MatPCUDA<PType, SType>& matp, PType& matp_value, MeshParam_List& ... params)
{
	if (matp.is_sdep()) {
		...
		cuReal3 position = pM->get_crelpos_from_relpos(pM->cellidx_to_position(mcell_idx));
...
{
	if (matp.is_sdep()) matp_value = matp.get(pM->get_crelpos_from_relpos(pM->cellidx_to_position(mcell_idx)), *pcuDiffEq->pstagetime);
}
...
__device__ void ManagedMeshCUDA::update_parameters_mcoarse_full(int mcell_idx, MatPCUDA<PType, SType>& matp, PType& matp_value)
{
	if (matp.is_sdep()) {
		...
		matp_value = matp.get(position, *pcuDiffEq->pstagetime, (*pTemp)[pTemp->get_relpos_from_crelpos(position)]);
	}
	else if (matp.is_tdep()) {
...
{
	if (matp.is_sdep()) matp_value = matp.get(pV->get_crelpos_from_relpos(pV->cellidx_to_position(ecell_idx)), *pcuDiffEq->pstagetime);
}
...
{
	if (matp.is_sdep()) matp_value = matp.get(pTemp->get_crelpos_from_relpos(pTemp->cellidx_to_position(tcell_idx)), *pcuDiffEq->pstagetime);
}
...
{
	if (matp.is_sdep()) matp_value = matp.get(pu_disp->get_crelpos_from_relpos(pu_disp->cellidx_to_position(scell_idx)), *pcuDiffEq->pstagetime);
}
