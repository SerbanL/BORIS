...
Any SuperMeshCUDA::GetAverageDisplayedMeshValue(std::string meshName, Rect rel_rect, std::vector<MeshShape> shapes, MESHDISPLAY_ quantity)
{
	...
	case MESHDISPLAY_SM_DEMAG:
		...
		if (pSMesh->IsSuperMeshModuleSet(MODS_SDEMAG)) {
			...
			return (DBL3)dynamic_cast<SDemag*>(pSMesh->pSMod(MODS_SDEMAG))->GetDemagFieldCUDA().average_nonempty((cuRect)rel_rect);
		}
		break;
	...
	case MESHDISPLAY_SM_OERSTED:
		...
		if (pSMesh->IsSuperMeshModuleSet(MODS_OERSTED)) {
			...
			return (DBL3)dynamic_cast<Oersted*>(pSMesh->pSMod(MODS_OERSTED))->GetOerstedFieldCUDA().average_nonempty((cuRect)rel_rect);
		}
		break;
	...
	case MESHDISPLAY_SM_STRAYH:
		...
		if (pSMesh->IsSuperMeshModuleSet(MODS_STRAYFIELD)) {
			...
			return (DBL3)dynamic_cast<StrayField*>(pSMesh->pSMod(MODS_STRAYFIELD))->GetStrayFieldCUDA().average_nonempty((cuRect)rel_rect);
		}
		break;
	}
