...
//N = (N.x/2 + 1, N.y, 1)
__global__ void cu_Demag_ConvProd_2D(
...
__global__ void cu_Demag_ConvProd_2D_transpose_xy(
	cuVEC<cuReal3>& Kdiag, cuVEC<cuBReal>& K2D_odiag, 
	cuBComplex* cuSx_in, cuBComplex* cuSy_in, cuBComplex* cuSz_in, 
	cuBComplex* cuSx_out, cuBComplex* cuSy_out, cuBComplex* cuSz_out, 
	cuSZ3& N_xRegion)
...
//N = (N.x/2 + 1, N.y, N.z)
__global__ void cu_Demag_ConvProd_3D_transpose_xy(
...
//N = (N.x/2 + 1, N.y, 4)
//xy is transposed
__global__ void cu_Demag_ConvProd_q2D_4_transpose_xy(
	cuVEC<cuReal3>& Kdiag, cuVEC<cuReal3>& Kodiag,
	cuBComplex* cuSx_in, cuBComplex* cuSy_in, cuBComplex* cuSz_in,
	cuBComplex* cuSx_out, cuBComplex* cuSy_out, cuBComplex* cuSz_out,
	cuSZ3& N_xRegion)
...
//N = (N.x/2 + 1, N.y, 8)
//xy is transposed
__global__ void cu_Demag_ConvProd_q2D_8_transpose_xy(
	cuVEC<cuReal3>& Kdiag, cuVEC<cuReal3>& Kodiag,
	cuBComplex* cuSx_in, cuBComplex* cuSy_in, cuBComplex* cuSz_in,
	cuBComplex* cuSx_out, cuBComplex* cuSy_out, cuBComplex* cuSz_out,
	cuSZ3& N_xRegion, cuSZ3& n)
{
	...
	//NOTE on use of n:
	//n.z can be less than N.z / 2 here (i.e. not a power of 2) - possible values here 3
	//In this case truncate output above n.z as we don't want to pollute cuS spaces otherwise if zero padding kernels are not used
	//Also don't read from cuS spaces above n.z to reduce memory access cost (they will be zero anyway)
...
//N = (N.x/2 + 1, N.y, 16)
//xy is transposed
__global__ void cu_Demag_ConvProd_q2D_16_transpose_xy(
	cuVEC<cuReal3>& Kdiag, cuVEC<cuReal3>& Kodiag,
	cuBComplex* cuSx_in, cuBComplex* cuSy_in, cuBComplex* cuSz_in,
	cuBComplex* cuSx_out, cuBComplex* cuSy_out, cuBComplex* cuSz_out,
	cuSZ3& N_xRegion, cuSZ3& n)
{
	...
	//NOTE on use of n:
	//n.z can be less than N.z / 2 here (i.e. not a power of 2) - possible values here 5, 6, 7
	//In this case truncate output above n.z as we don't want to pollute cuS spaces otherwise if zero padding kernels are not used
	//Also don't read from cuS spaces above n.z to reduce memory access cost (they will be zero anyway)
...
//N = (N.x/2 + 1, N.y, 32)
//xy is transposed
__global__ void cu_Demag_ConvProd_q2D_32_transpose_xy(
	cuVEC<cuReal3>& Kdiag, cuVEC<cuReal3>& Kodiag,
	cuBComplex* cuSx_in, cuBComplex* cuSy_in, cuBComplex* cuSz_in,
	cuBComplex* cuSx_out, cuBComplex* cuSy_out, cuBComplex* cuSz_out,
	cuSZ3& N_xRegion, cuSZ3& n)
{
	...
	//NOTE on use of n:
	//n.z can be less than N.z / 2 here (i.e. not a power of 2) - possible values here 9, 10, 11, 12, 13, 14, 15
	//In this case truncate output above n.z as we don't want to pollute cuS spaces otherwise if zero padding kernels are not used
	//Also don't read from cuS spaces above n.z to reduce memory access cost (they will be zero anyway)
...
void DemagKernelCUDA::KernelMultiplication_2D(void)
{
	if (transpose_xy) {
		...
		if (!additional_spaces) {
			...
			cu_Demag_ConvProd_2D_transpose_xy <<< (nxRegion * N.y + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
		...
		if (!additional_spaces) {
			...
			cu_Demag_ConvProd_2D <<< (nxRegion * N.y + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> 
				(Kdiag, K2D_odiag, cuS_x, cuS_y, cuS_z, cuS_x, cuS_y, cuS_z, cuN_xRegion);
		}
		else {
...
void DemagKernelCUDA::KernelMultiplication_3D(void)
{
	...
	if (!additional_spaces) {
		...
		cu_Demag_ConvProd_3D_transpose_xy <<< (nxRegion * N.y * N.z + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>>
...
void DemagKernelCUDA::KernelMultiplication_q2D(int q2D_level)
{
	...
	case 4:
		if (!additional_spaces) {
			...
			cu_Demag_ConvProd_q2D_4_transpose_xy <<< (nxRegion * N.y + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (Kdiag, Kodiag, cuS_x, cuS_y, cuS_z, cuS_x, cuS_y, cuS_z, cuN_xRegion);
		}
		else {

			cu_Demag_ConvProd_q2D_4_transpose_xy <<< (nxRegion * N.y + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (Kdiag, Kodiag, cuS_x, cuS_y, cuS_z, cuS2_x, cuS2_y, cuS2_z, cuN_xRegion);
		}
		break;
	...
	case 8:
		if (!additional_spaces) {
			...
			cu_Demag_ConvProd_q2D_8_transpose_xy <<< (nxRegion * N.y + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (Kdiag, Kodiag, cuS_x, cuS_y, cuS_z, cuS_x, cuS_y, cuS_z, cuN_xRegion, cun);
		}
		else {

			cu_Demag_ConvProd_q2D_8_transpose_xy <<< (nxRegion * N.y + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (Kdiag, Kodiag, cuS_x, cuS_y, cuS_z, cuS2_x, cuS2_y, cuS2_z, cuN_xRegion, cun);
		}
		break;
	...
	case 16:
		if (!additional_spaces) {
			...
			cu_Demag_ConvProd_q2D_16_transpose_xy <<< (nxRegion * N.y + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (Kdiag, Kodiag, cuS_x, cuS_y, cuS_z, cuS_x, cuS_y, cuS_z, cuN_xRegion, cun);
		}
		else {

			cu_Demag_ConvProd_q2D_16_transpose_xy <<< (nxRegion * N.y + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (Kdiag, Kodiag, cuS_x, cuS_y, cuS_z, cuS2_x, cuS2_y, cuS2_z, cuN_xRegion, cun);
		}
		break;
	...
	case 32:
		if (!additional_spaces) {
			...
			cu_Demag_ConvProd_q2D_32_transpose_xy <<< (nxRegion * N.y + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (Kdiag, Kodiag, cuS_x, cuS_y, cuS_z, cuS_x, cuS_y, cuS_z, cuN_xRegion, cun);
		}
		else {

			cu_Demag_ConvProd_q2D_32_transpose_xy <<< (nxRegion * N.y + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (Kdiag, Kodiag, cuS_x, cuS_y, cuS_z, cuS2_x, cuS2_y, cuS2_z, cuN_xRegion, cun);
		}
		break;
