...
__global__ static void extract_halony_kernel(const cuSZ3& n, VType*& quantity, VType*& halo, size_t& halo_size)
{
	...
	if (box_idx < halo_size) {
		...
		//now form index in quantity
		int quant_idx = box_i + (box_j + n.y - halo_depth) * n.x + box_k * n.x*n.y;
...
__host__ void cuVEC_VC<VType>::extract_halo(int halo_id, size_t size_transfer)
{
	...
	case NF2_HALONX:
		extract_halonx_kernel <<< (size_transfer + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (cuVEC<VType>::n, cuVEC<VType>::quantity, halotemp_n, halo_size);
		break;
	case NF2_HALOPX:
		extract_halopx_kernel <<< (size_transfer + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (cuVEC<VType>::n, cuVEC<VType>::quantity, halotemp_p, halo_size);
		break;
	case NF2_HALONY:
		extract_halony_kernel <<< (size_transfer + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (cuVEC<VType>::n, cuVEC<VType>::quantity, halotemp_n, halo_size);
		break;
	case NF2_HALOPY:
		extract_halopy_kernel <<< (size_transfer + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (cuVEC<VType>::n, cuVEC<VType>::quantity, halotemp_p, halo_size);
		break;
	case NF2_HALONZ:
		extract_halonz_kernel <<< (size_transfer + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (cuVEC<VType>::n, cuVEC<VType>::quantity, halotemp_n, halo_size);
		break;
	case NF2_HALOPZ:
		extract_halopz_kernel <<< (size_transfer + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (cuVEC<VType>::n, cuVEC<VType>::quantity, halotemp_p, halo_size);
		break;
	}
}