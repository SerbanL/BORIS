...
__global__ void strided_copy_3d(VType*& quantity_in, int*& ngbrFlags_in, cuSZ3& n, VType*& quantity_out, int*& ngbrFlags_out, cuSZ3& n_coarse, cuBReal multiplier)
{
...
template void cuVEC_VC<float>::extract_cuvec(size_t size, cuVEC_VC<float>& cuvec, cuBReal multiplier);
template void cuVEC_VC<double>::extract_cuvec(size_t size, cuVEC_VC<double>& cuvec, cuBReal multiplier);

template void cuVEC_VC<cuFLT3>::extract_cuvec(size_t size, cuVEC_VC<cuFLT3>& cuvec, cuBReal multiplier);
template void cuVEC_VC<cuDBL3>::extract_cuvec(size_t size, cuVEC_VC<cuDBL3>& cuvec, cuBReal multiplier);

template <typename VType>
__host__ void cuVEC_VC<VType>::extract_cuvec(size_t size, cuVEC_VC<VType>& cuvec, cuBReal multiplier)
{
	strided_copy_3d <<< (size + CUDATHREADS) / CUDATHREADS, CUDATHREADS >>> (cuVEC<VType>::quantity, ngbrFlags, cuVEC<VType>::n, cuvec.quantity, cuvec.ngbrFlags, cuvec.n, multiplier);
}