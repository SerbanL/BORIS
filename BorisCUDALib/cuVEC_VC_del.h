...
__device__ VType cuVEC_VC<VType>::delsq_neu(int idx) const
{
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPX) {
			...
			if (ngbrFlags[idx] & NF_NNX) diff_x = (halo_val + cuVEC<VType>::quantity[idx - 1] - 2 * cuVEC<VType>::quantity[idx]);
			...
			if (ngbrFlags[idx] & NF_NPX) diff_x = (cuVEC<VType>::quantity[idx + 1] + halo_val - 2 * cuVEC<VType>::quantity[idx]);
			else diff_x = (halo_val - cuVEC<VType>::quantity[idx]);
		}
	}
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPY) {
			...
			if (ngbrFlags[idx] & NF_NNY) diff_y = (halo_val + cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x] - 2 * cuVEC<VType>::quantity[idx]);
			...
			if (ngbrFlags[idx] & NF_NPY) diff_y = (cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x] + halo_val - 2 * cuVEC<VType>::quantity[idx]);
			else diff_y = (halo_val - cuVEC<VType>::quantity[idx]);
		}
	}
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPZ) {
			...
			if (ngbrFlags[idx] & NF_NNZ) diff_z = (halo_val + cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x*cuVEC<VType>::n.y] - 2 * cuVEC<VType>::quantity[idx]);
			...
			if (ngbrFlags[idx] & NF_NPZ) diff_z = (cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x*cuVEC<VType>::n.y] + halo_val - 2 * cuVEC<VType>::quantity[idx]);
			else diff_z = (halo_val - cuVEC<VType>::quantity[idx]);
		}
	}
		...
		if (ngbrFlags2[idx] & NF2_HALOPX) {
			...
			if (ngbrFlags[idx] & NF_NNX) diff_x = (halo_val + cuVEC<VType>::quantity[idx - 1] - 2 * cuVEC<VType>::quantity[idx]);
				...
				if (ngbrFlags2[idx] & NF2_ROBINV) {
					...
					diff_x = ((1 + robin_v.i * cuVEC<VType>::h.x / (2 * K)) * halo_val + robin_v.i * cuVEC<VType>::h.x * robin_v.j / K - (1 + 3 * robin_v.i * cuVEC<VType>::h.x / (2 * K)) * cuVEC<VType>::quantity[idx]);
				}
				else diff_x = (halo_val - cuVEC<VType>::quantity[idx]);
			}
		}
		...
		if (ngbrFlags2[idx] & NF2_HALOPY) {
			...
			if (ngbrFlags[idx] & NF_NNY) diff_y = (halo_val + cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x] - 2 * cuVEC<VType>::quantity[idx]);
				...
				if (ngbrFlags2[idx] & NF2_ROBINV) {
					...
					diff_y = ((1 + robin_v.i * cuVEC<VType>::h.y / (2 * K)) * halo_val + robin_v.i * cuVEC<VType>::h.y * robin_v.j / K - (1 + 3 * robin_v.i * cuVEC<VType>::h.y / (2 * K)) * cuVEC<VType>::quantity[idx]);
				}
				else diff_y = (halo_val - cuVEC<VType>::quantity[idx]);
			}
		}
		...
		if (ngbrFlags2[idx] & NF2_HALOPZ) {
			...
			if (ngbrFlags[idx] & NF_NNZ) diff_z = (halo_val + cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x*cuVEC<VType>::n.y] - 2 * cuVEC<VType>::quantity[idx]);
				...
				if (ngbrFlags2[idx] & NF2_ROBINV) {
					...
					diff_z = ((1 + robin_v.i * cuVEC<VType>::h.z / (2 * K)) * halo_val + robin_v.i * cuVEC<VType>::h.z * robin_v.j / K - (1 + 3 * robin_v.i * cuVEC<VType>::h.z / (2 * K)) * cuVEC<VType>::quantity[idx]);
				}
				else diff_z = (halo_val - cuVEC<VType>::quantity[idx]);
			}
		}
