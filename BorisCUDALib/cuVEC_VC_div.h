...
__device__ cuBReal cuVEC_VC<VType>::div_neu(int idx) const
{
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPX) {
			...
			if (ngbrFlags[idx] & NF_NNX) div += (halo_val.x - cuVEC<VType>::quantity[idx - 1].x) / (2 * cuVEC<VType>::h.x);
			else div += (halo_val.x - cuVEC<VType>::quantity[idx].x) / (2 * cuVEC<VType>::h.x);
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPX) div += (cuVEC<VType>::quantity[idx + 1].x - halo_val.x) / (2 * cuVEC<VType>::h.x);
			else div += (cuVEC<VType>::quantity[idx].x - halo_val.x) / (2 * cuVEC<VType>::h.x);
		}
	}
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPY) {
			...
			if (ngbrFlags[idx] & NF_NNY) div += (halo_val.y - cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x].y) / (2 * cuVEC<VType>::h.y);
			else div += (halo_val.y - cuVEC<VType>::quantity[idx].y) / (2 * cuVEC<VType>::h.y);
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPY) div += (cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x].y - halo_val.y) / (2 * cuVEC<VType>::h.y);
			else div += (cuVEC<VType>::quantity[idx].y - halo_val.y) / (2 * cuVEC<VType>::h.y);
		}
	}
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPZ) {
			...
			if (ngbrFlags[idx] & NF_NNZ) div += (halo_val.z - cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x*cuVEC<VType>::n.y].z) / (2 * cuVEC<VType>::h.z);
			else div += (halo_val.z - cuVEC<VType>::quantity[idx].z) / (2 * cuVEC<VType>::h.z);
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPZ) div += (cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x*cuVEC<VType>::n.y].z - halo_val.z) / (2 * cuVEC<VType>::h.z);
			else div += (cuVEC<VType>::quantity[idx].z - halo_val.z) / (2 * cuVEC<VType>::h.z);
		}
	}
...
__device__ cuBReal cuVEC_VC<VType>::div_nneu(int idx, const Class_BDiff& bdiff_class) const
{
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPX) {
			...
			if (ngbrFlags[idx] & NF_NNX) div += (halo_val.x - cuVEC<VType>::quantity[idx - 1].x) / (2 * cuVEC<VType>::h.x);
			else div += ((halo_val.x - cuVEC<VType>::quantity[idx].x) / cuVEC<VType>::h.x + bdiff_class.bdiff(idx).x.x) / 2;
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPX) div += (cuVEC<VType>::quantity[idx + 1].x - halo_val.x) / (2 * cuVEC<VType>::h.x);
			else div += ((cuVEC<VType>::quantity[idx].x - halo_val.x) / cuVEC<VType>::h.x + bdiff_class.bdiff(idx).x.x) / 2;
		}
	}
	//Is it a CMBND boundary? - if not then use homogeneous Neumann condition (differential zero at the boundary)
	else if (ngbrFlags[idx] & NF_CMBNDX) {
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPY) {
			...
			if (ngbrFlags[idx] & NF_NNY) div += (halo_val.y - cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x].y) / (2 * cuVEC<VType>::h.y);
			else div += ((halo_val.y - cuVEC<VType>::quantity[idx].y) / cuVEC<VType>::h.y + bdiff_class.bdiff(idx).y.y) / 2;
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPY) div += (cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x].y - halo_val.y) / (2 * cuVEC<VType>::h.y);
			else div += ((cuVEC<VType>::quantity[idx].y - halo_val.y) / cuVEC<VType>::h.y + bdiff_class.bdiff(idx).y.y) / 2;
		}
	}
	else if (ngbrFlags[idx] & NF_CMBNDY) {
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPZ) {
			...
			if (ngbrFlags[idx] & NF_NNZ) div += (halo_val.z - cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x*cuVEC<VType>::n.y].z) / (2 * cuVEC<VType>::h.z);
			else div += ((halo_val.z - cuVEC<VType>::quantity[idx].z) / cuVEC<VType>::h.z + bdiff_class.bdiff(idx).z.z) / 2;
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPZ) div += (cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x*cuVEC<VType>::n.y].z - halo_val.z) / (2 * cuVEC<VType>::h.z);
			else div += ((cuVEC<VType>::quantity[idx].z - halo_val.z) / cuVEC<VType>::h.z + bdiff_class.bdiff(idx).z.z) / 2;
		}
	}
	else if (ngbrFlags[idx] & NF_CMBNDZ) {
...
__device__ cuBReal cuVEC_VC<VType>::div_nneu(int idx, const cuVAL3<VType>& bdiff) const
{
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPX) {
			...
			if (ngbrFlags[idx] & NF_NNX) div += (halo_val.x - cuVEC<VType>::quantity[idx - 1].x) / (2 * cuVEC<VType>::h.x);
			else div += ((halo_val.x - cuVEC<VType>::quantity[idx].x) / cuVEC<VType>::h.x + bdiff.x.x) / 2;
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPX) div += (cuVEC<VType>::quantity[idx + 1].x - halo_val.x) / (2 * cuVEC<VType>::h.x);
			else div += ((cuVEC<VType>::quantity[idx].x - halo_val.x) / cuVEC<VType>::h.x + bdiff.x.x) / 2;
		}
	}
	//Is it a CMBND boundary? - if not then use homogeneous Neumann condition (differential zero at the boundary)
	else if (ngbrFlags[idx] & NF_CMBNDX) {
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPY) {
			...
			if (ngbrFlags[idx] & NF_NNY) div += (halo_val.y - cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x].y) / (2 * cuVEC<VType>::h.y);
			else div += ((halo_val.y - cuVEC<VType>::quantity[idx].y) / cuVEC<VType>::h.y + bdiff.y.y) / 2;
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPY) div += (cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x].y - halo_val.y) / (2 * cuVEC<VType>::h.y);
			else div += ((cuVEC<VType>::quantity[idx].y - halo_val.y) / cuVEC<VType>::h.y + bdiff.y.y) / 2;
		}
	}
	else if (ngbrFlags[idx] & NF_CMBNDY) {
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPZ) {
			...
			if (ngbrFlags[idx] & NF_NNZ) div += (halo_val.z - cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x*cuVEC<VType>::n.y].z) / (2 * cuVEC<VType>::h.z);
			else div += ((halo_val.z - cuVEC<VType>::quantity[idx].z) / cuVEC<VType>::h.z + bdiff.z.z) / 2;
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPZ) div += (cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x*cuVEC<VType>::n.y].z - halo_val.z) / (2 * cuVEC<VType>::h.z);
			else div += ((cuVEC<VType>::quantity[idx].z - halo_val.z) / cuVEC<VType>::h.z + bdiff.z.z) / 2;
		}
	}
	else if (ngbrFlags[idx] & NF_CMBNDZ) {
...
__device__ cuBReal cuVEC_VC<VType>::div_sided(int idx) const
{
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPX) {
			...
			if (ngbrFlags[idx] & NF_NNX) div += (halo_val.x - cuVEC<VType>::quantity[idx - 1].x) / (2 * cuVEC<VType>::h.x);
			else div += (halo_val.x - cuVEC<VType>::quantity[idx].x) / cuVEC<VType>::h.x;
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPX) div += (cuVEC<VType>::quantity[idx + 1].x - halo_val.x) / (2 * cuVEC<VType>::h.x);
			else div += (cuVEC<VType>::quantity[idx].x - halo_val.x) / cuVEC<VType>::h.x;
		}
	}
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPY) {
			...
			if (ngbrFlags[idx] & NF_NNY) div += (halo_val.y - cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x].y) / (2 * cuVEC<VType>::h.y);
			else div += (halo_val.y - cuVEC<VType>::quantity[idx].y) / cuVEC<VType>::h.y;
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPY) div += (cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x].y - halo_val.y) / (2 * cuVEC<VType>::h.y);
			else div += (cuVEC<VType>::quantity[idx].y - halo_val.y) / cuVEC<VType>::h.y;
		}
	}
	else if (ngbrFlags[idx] & NF_NGBRY) {
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPZ) {
			...
			if (ngbrFlags[idx] & NF_NNZ) div += (halo_val.z - cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x*cuVEC<VType>::n.y].z) / (2 * cuVEC<VType>::h.z);
			else div += (halo_val.z - cuVEC<VType>::quantity[idx].z) / cuVEC<VType>::h.z;
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPZ) div += (cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x*cuVEC<VType>::n.y].z - halo_val.z) / (2 * cuVEC<VType>::h.z);
			else div += (cuVEC<VType>::quantity[idx].z - halo_val.z) / cuVEC<VType>::h.z;
		}
	}
	else if (ngbrFlags[idx] & NF_NGBRZ) {
...
__device__ VType cuVEC_VC<VType>::diveps3_neu(int idx) const
{
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPX) {
			...
			if (ngbrFlags[idx] & NF_NNX) diffx = (halo_val - cuVEC<VType>::quantity[idx - 1]) / (2 * cuVEC<VType>::h.x);
			else diffx = (halo_val - cuVEC<VType>::quantity[idx]) / (2 * cuVEC<VType>::h.x);
		}
		else {

			VType halo_val = (pUVA_haloVEC_left ? (*pUVA_haloVEC_left)[(pUVA_haloVEC_left->n.x - 1) + j * pUVA_haloVEC_left->n.x + k * pUVA_haloVEC_left->n.x*pUVA_haloVEC_left->n.y] : halo_n[j + k * cuVEC<VType>::n.y]);

			if (ngbrFlags[idx] & NF_NPX) diffx = (cuVEC<VType>::quantity[idx + 1] - halo_val) / (2 * cuVEC<VType>::h.x);
			else diffx = (cuVEC<VType>::quantity[idx] - halo_val) / (2 * cuVEC<VType>::h.x);
		}
	}
	//Is it a CMBND boundary? - if not then use homogeneous Neumann condition (differential zero at the boundary)
	else if (ngbrFlags[idx] & NF_CMBNDX) {
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPY) {
			...
			if (ngbrFlags[idx] & NF_NNY) diffy = (halo_val - cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x]) / (2 * cuVEC<VType>::h.y);
			else diffy = (halo_val - cuVEC<VType>::quantity[idx]) / (2 * cuVEC<VType>::h.y);
		}
		else {

			VType halo_val = (pUVA_haloVEC_left ? (*pUVA_haloVEC_left)[i + (pUVA_haloVEC_left->n.y - 1)*pUVA_haloVEC_left->n.x + k * pUVA_haloVEC_left->n.x*pUVA_haloVEC_left->n.y] : halo_n[i + k * cuVEC<VType>::n.x]);

			if (ngbrFlags[idx] & NF_NPY) diffy = (cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x] - halo_val) / (2 * cuVEC<VType>::h.y);
			else diffy = (cuVEC<VType>::quantity[idx] - halo_val) / (2 * cuVEC<VType>::h.y);
		}
	}
	else if (ngbrFlags[idx] & NF_CMBNDY) {
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPZ) {
			...
			if (ngbrFlags[idx] & NF_NNZ) diffz = (halo_val - cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x*cuVEC<VType>::n.y]) / (2 * cuVEC<VType>::h.z);
			else diffz = (halo_val - cuVEC<VType>::quantity[idx]) / (2 * cuVEC<VType>::h.z);
		}
		else {

			VType halo_val = (pUVA_haloVEC_left ? (*pUVA_haloVEC_left)[i + j * pUVA_haloVEC_left->n.x + (pUVA_haloVEC_left->n.z - 1)*pUVA_haloVEC_left->n.x*pUVA_haloVEC_left->n.y] : halo_n[i + j * cuVEC<VType>::n.x]);

			if (ngbrFlags[idx] & NF_NPZ) diffz = (cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x*cuVEC<VType>::n.y] - halo_val) / (2 * cuVEC<VType>::h.z);
			else diffz = (cuVEC<VType>::quantity[idx] - halo_val) / (2 * cuVEC<VType>::h.z);
		}
	}
	else if (ngbrFlags[idx] & NF_CMBNDZ) {
...
__device__ VType cuVEC_VC<VType>::diveps3_sided(int idx) const
{
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPX) {
			...
			if (ngbrFlags[idx] & NF_NNX) diffx = (halo_val - cuVEC<VType>::quantity[idx - 1]) / (2 * cuVEC<VType>::h.x);
			else diffx = (halo_val - cuVEC<VType>::quantity[idx]) / cuVEC<VType>::h.x;
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPX) diffx = (cuVEC<VType>::quantity[idx + 1] - halo_val) / (2 * cuVEC<VType>::h.x);
			else diffx = (cuVEC<VType>::quantity[idx] - halo_val) / cuVEC<VType>::h.x;
		}
	}
	//use sided differentials if one of the neighbors is present
	else if (ngbrFlags[idx] & NF_NGBRX) {
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPY) {
			...
			if (ngbrFlags[idx] & NF_NNY) diffy = (halo_val - cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x]) / (2 * cuVEC<VType>::h.y);
			else diffy = (halo_val - cuVEC<VType>::quantity[idx]) / cuVEC<VType>::h.y;
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPY) diffy = (cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x] - halo_val) / (2 * cuVEC<VType>::h.y);
			else diffy = (cuVEC<VType>::quantity[idx] - halo_val) / cuVEC<VType>::h.y;
		}
	}
	else if (ngbrFlags[idx] & NF_NGBRY) {
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPZ) {
			...
			if (ngbrFlags[idx] & NF_NNZ) diffz = (halo_val - cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x*cuVEC<VType>::n.y]) / (2 * cuVEC<VType>::h.z);
			else diffz = (halo_val - cuVEC<VType>::quantity[idx]) / cuVEC<VType>::h.z;
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPZ) diffz = (cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x*cuVEC<VType>::n.y] - halo_val) / (2 * cuVEC<VType>::h.z);
			else diffz = (cuVEC<VType>::quantity[idx] - halo_val) / cuVEC<VType>::h.z;
		}
	}
	else if (ngbrFlags[idx] & NF_NGBRZ) {
