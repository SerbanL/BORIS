...
__global__ void set_ngbrFlags_kernel(const cuSZ3& n, int*& ngbrFlags, VType*& quantity, int& nonempty_cells)
{
	...
	if (idx < n.dim()) {
		...
		//first clear all neighbor existence flags (bits 0 to 5 and 17 to 31 inclusive)
		ngbrFlags[idx] &= ~(NF_BOTHX + NF_BOTHY + NF_BOTHZ + NF_XY_FULL + NF_XZ_FULL + NF_YZ_FULL + NF_XY_OASTENCIL + NF_XZ_OASTENCIL + NF_YZ_OASTENCIL);
...
__host__ bool cuVEC_VC<VType>::set_halo_conditions(cu_arr<int>& halo_ngbrFlags, int halo_flag, int halo_depth)
{
	...
	case NF2_HALOPX:
	case NF2_HALONX:
		halo_size_cpu = halo_depth * n_cpu.y * n_cpu.z;
		break;

	case NF2_HALOPY:
	case NF2_HALONY:
		halo_size_cpu = halo_depth * n_cpu.x * n_cpu.z;
		break;

	case NF2_HALOPZ:
	case NF2_HALONZ:
		halo_size_cpu = halo_depth * n_cpu.x * n_cpu.y;
		break;
	}

	//error: size mismatch
	if (halo_size_cpu != halo_ngbrFlags.size()) return false;

	cudaError_t error = set_halo_size(halo_size_cpu);
	if (error == cudaSuccess) {
		...
		if (ngbrFlags[idx] & NF_NOTEMPTY) {
			...
			if (ijk.i < n.x - 1) {
				...
				if (!(ngbrFlags[idx + 1] & NF_NOTEMPTY))
					...
					if (cuIsNZ(robin_v.i) && ((ijk.i > 0 && (ngbrFlags[idx - 1] & NF_NOTEMPTY)) || (ngbrFlags2[idx] & NF2_HALONX))) { ngbrFlags2[idx] |= (NF2_ROBINNX + NF2_ROBINV); }
			}
			...
			if (ijk.i > 0) {
				...
				if (!(ngbrFlags[idx - 1] & NF_NOTEMPTY))
					if (cuIsNZ(robin_v.i) && ((ijk.i < n.x - 1 && (ngbrFlags[idx + 1] & NF_NOTEMPTY)) || (ngbrFlags2[idx] & NF2_HALOPX))) { ngbrFlags2[idx] |= (NF2_ROBINPX + NF2_ROBINV); }
			}
			...
			if (ijk.j < n.y - 1) {
				...
				if (!(ngbrFlags[idx + n.x] & NF_NOTEMPTY))
					if (cuIsNZ(robin_v.i) && ((ijk.j > 0 && (ngbrFlags[idx - n.x] & NF_NOTEMPTY)) || (ngbrFlags2[idx] & NF2_HALONY))) { ngbrFlags2[idx] |= (NF2_ROBINNY + NF2_ROBINV); }
			}
			...
			if (ijk.j > 0) {
				...
				if (!(ngbrFlags[idx - n.x] & NF_NOTEMPTY))
					if (cuIsNZ(robin_v.i) && ((ijk.j < n.y - 1 && (ngbrFlags[idx + n.x] & NF_NOTEMPTY)) || (ngbrFlags2[idx] & NF2_HALOPY))) { ngbrFlags2[idx] |= (NF2_ROBINPY + NF2_ROBINV); }
			}
			...
			if (ijk.k < n.z - 1) {
				...
				if (!(ngbrFlags[idx + n.x*n.y] & NF_NOTEMPTY))
					if (cuIsNZ(robin_v.i) && ((ijk.k > 0 && (ngbrFlags[idx - n.x*n.y] & NF_NOTEMPTY)) || (ngbrFlags2[idx] & NF2_HALONZ))) { ngbrFlags2[idx] |= (NF2_ROBINNZ + NF2_ROBINV); }
			}
			...
			if (ijk.k > 0) {
				...
				if (!(ngbrFlags[idx - n.x*n.y] & NF_NOTEMPTY))
					if (cuIsNZ(robin_v.i) && ((ijk.k < n.z - 1 && (ngbrFlags[idx + n.x*n.y] & NF_NOTEMPTY)) || (ngbrFlags2[idx] & NF2_HALOPZ))) { ngbrFlags2[idx] |= (NF2_ROBINPZ + NF2_ROBINV); }
			}
