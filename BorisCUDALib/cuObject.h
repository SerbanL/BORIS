...
public:
	...
	cu_obj(void)
	{
		...
		call_constructor(std::integral_constant<bool, std::is_fundamental<MType>::value || std::is_same<MType, cuBComplex>::value >());
	}
	...
	cu_obj(PType ... params)
	{
		...
		//call "managed" constructor - managed object has to implement this, where it will allocate gpu memory for its stored types
		call_constructor(std::integral_constant<bool, std::is_fundamental<MType>::value || std::is_same<MType, cuBComplex>::value >(), params...);
	}
	...
	~cu_obj()
	{
		...
		if (managed_cuda_object) {
			...
			call_destructor(std::integral_constant<bool, std::is_fundamental<MType>::value || std::is_same<MType, cuBComplex>::value >());
	...
	cu_obj(const cu_obj& copyThis)
	{
		...
		call_constructor_ref(std::integral_constant<bool, std::is_fundamental<MType>::value || std::is_same<MType, cuBComplex>::value >(), *(copyThis.managed_cuda_object));
	}
	...
	cu_obj& operator=(const cu_obj& copyThis)
	{
		...
		call_assignment(std::integral_constant<bool, std::is_fundamental<MType>::value || std::is_same<MType, cuBComplex>::value >(), *(copyThis.managed_cuda_object));
