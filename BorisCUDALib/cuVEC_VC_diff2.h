...
__device__ VType cuVEC_VC<VType>::dxx_neu(int idx) const
{
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPX) {
			...
			if (ngbrFlags[idx] & NF_NNX) diff = (halo_val + cuVEC<VType>::quantity[idx - 1] - 2 * cuVEC<VType>::quantity[idx]) / hsq;
			else diff = (halo_val - cuVEC<VType>::quantity[idx]) / hsq;
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPX) diff = (cuVEC<VType>::quantity[idx + 1] + halo_val - 2 * cuVEC<VType>::quantity[idx]) / hsq;
			else diff = (halo_val - cuVEC<VType>::quantity[idx]) / hsq;
		}
	}
...
__device__ VType cuVEC_VC<VType>::dyy_neu(int idx) const
{
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPY) {
			...
			if (ngbrFlags[idx] & NF_NNY) diff = (halo_val + cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x] - 2 * cuVEC<VType>::quantity[idx]) / hsq;
			else diff = (halo_val - cuVEC<VType>::quantity[idx]) / hsq;
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPY) diff = (cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x] + halo_val - 2 * cuVEC<VType>::quantity[idx]) / hsq;
			else diff = (halo_val - cuVEC<VType>::quantity[idx]) / hsq;
		}
	}
...
__device__ VType cuVEC_VC<VType>::dzz_neu(int idx) const
{
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPZ) {
			...
			if (ngbrFlags[idx] & NF_NNZ) diff = (halo_val + cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x*cuVEC<VType>::n.y] - 2 * cuVEC<VType>::quantity[idx]) / hsq;
			else diff = (halo_val - cuVEC<VType>::quantity[idx]) / hsq;
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPZ) diff = (cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x*cuVEC<VType>::n.y] + halo_val - 2 * cuVEC<VType>::quantity[idx]) / hsq;
			else diff = (halo_val - cuVEC<VType>::quantity[idx]) / hsq;
		}
	}
...
__device__ VType cuVEC_VC<VType>::dxx_nneu(int idx, const Class_BDiff& bdiff_class) const
{
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPX) {
			...
			if (ngbrFlags[idx] & NF_NNX) diff = (halo_val + cuVEC<VType>::quantity[idx - 1] - 2 * cuVEC<VType>::quantity[idx]) / hsq;
			else diff = ((halo_val - cuVEC<VType>::quantity[idx]) - bdiff_class.bdiff(idx).x * cuVEC<VType>::h.x) / hsq;
		}
		else {
...
__device__ VType cuVEC_VC<VType>::dyy_nneu(int idx, const Class_BDiff& bdiff_class) const
{
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPY) {
			...
			if (ngbrFlags[idx] & NF_NNY) diff = (halo_val + cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x] - 2 * cuVEC<VType>::quantity[idx]) / hsq;
			else diff = diff = ((halo_val - cuVEC<VType>::quantity[idx]) - bdiff_class.bdiff(idx).y * cuVEC<VType>::h.y) / hsq;
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPY) diff = (cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x] + halo_val - 2 * cuVEC<VType>::quantity[idx]) / hsq;
			else diff = ((halo_val - cuVEC<VType>::quantity[idx]) + bdiff_class.bdiff(idx).y * cuVEC<VType>::h.y) / hsq;
		}
	}
	else if (ngbrFlags[idx] & NF_CMBNDY) {
...
__device__ VType cuVEC_VC<VType>::dzz_nneu(int idx, const Class_BDiff& bdiff_class) const
{
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPZ) {
			...
			if (ngbrFlags[idx] & NF_NNZ) diff = (halo_val + cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x*cuVEC<VType>::n.y] - 2 * cuVEC<VType>::quantity[idx]) / hsq;
			else diff = ((halo_val - cuVEC<VType>::quantity[idx]) - bdiff_class.bdiff(idx).z * cuVEC<VType>::h.z) / hsq;
		}
		else {
...
__device__ VType cuVEC_VC<VType>::dxx_nneu(int idx, const cuVAL3<VType>& bdiff) const
{
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPX) {
			...
			if (ngbrFlags[idx] & NF_NNX) diff = (halo_val + cuVEC<VType>::quantity[idx - 1] - 2 * cuVEC<VType>::quantity[idx]) / hsq;
			else diff = diff = ((halo_val - cuVEC<VType>::quantity[idx]) - bdiff.x * cuVEC<VType>::h.x) / hsq;
		}
		else {
			...
			if (ngbrFlags[idx] & NF_NPX) diff = (cuVEC<VType>::quantity[idx + 1] + halo_val - 2 * cuVEC<VType>::quantity[idx]) / hsq;
			else diff = ((halo_val - cuVEC<VType>::quantity[idx]) + bdiff.x * cuVEC<VType>::h.x) / hsq;
		}
	}
	else if (ngbrFlags[idx] & NF_CMBNDX) {
...
__device__ VType cuVEC_VC<VType>::dyy_nneu(int idx, const cuVAL3<VType>& bdiff) const
{
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPY) {
			...
			if (ngbrFlags[idx] & NF_NNY) diff = (halo_val + cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x] - 2 * cuVEC<VType>::quantity[idx]) / hsq;
			else diff = ((halo_val - cuVEC<VType>::quantity[idx]) - bdiff.y * cuVEC<VType>::h.y) / hsq;
		}
		else {
...
__device__ VType cuVEC_VC<VType>::dzz_nneu(int idx, const cuVAL3<VType>& bdiff) const
{
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPZ) {
			...
			if (ngbrFlags[idx] & NF_NNZ) diff = (halo_val + cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x*cuVEC<VType>::n.y] - 2 * cuVEC<VType>::quantity[idx]) / hsq;
			else diff = ((halo_val - cuVEC<VType>::quantity[idx]) - bdiff.z * cuVEC<VType>::h.z) / hsq;
		}
		else {
...
__device__ VType cuVEC_VC<VType>::dxx_diri_nneu(int idx, const Class_BDiff& bdiff_class) const
{
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPX) {
			...
			if (ngbrFlags[idx] & NF_NPX) diff = (cuVEC<VType>::quantity[idx + 1] + halo_val - 2 * cuVEC<VType>::quantity[idx]) / hsq;
			else ((halo_val - cuVEC<VType>::quantity[idx]) + bdiff_class.bdiff(idx).x * cuVEC<VType>::h.x) / hsq;
		}
	}
	else if (ngbrFlags[idx] & NF_CMBNDX) {
	...
	//use halo region?
		...
		if (ngbrFlags2[idx] & NF2_HALOPX) {
			...
			if (ngbrFlags[idx] & NF_NNX) diff = (halo_val + cuVEC<VType>::quantity[idx - 1] - 2 * cuVEC<VType>::quantity[idx]) / hsq;
				...
				//no neighbor on -ve side: is current cell marked with a robin flag (which would be on +ve side of boundary)? can only be ROBINV type as this is an inner cell in the larger cuVEC
				if (ngbrFlags2[idx] & NF2_ROBINV) {

					diff = ((1 + robin_v.i * cuVEC<VType>::h.x / (2 * K)) * halo_val + robin_v.i * cuVEC<VType>::h.x * robin_v.j / K - (1 + 3 * robin_v.i * cuVEC<VType>::h.x / (2 * K)) * cuVEC<VType>::quantity[idx]) / hsq;
				}
				else diff = (halo_val - cuVEC<VType>::quantity[idx]) / hsq;
			}
		}
		...
		if (ngbrFlags2[idx] & NF2_HALOPY) {
			...
			if (ngbrFlags[idx] & NF_NNY) diff = (halo_val + cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x] - 2 * cuVEC<VType>::quantity[idx]) / hsq;
				...
				if (ngbrFlags2[idx] & NF2_ROBINV) {
					...
					diff = ((1 + robin_v.i * cuVEC<VType>::h.y / (2 * K)) * halo_val + robin_v.i * cuVEC<VType>::h.y * robin_v.j / K - (1 + 3 * robin_v.i * cuVEC<VType>::h.y / (2 * K)) * cuVEC<VType>::quantity[idx]) / hsq;
				}
				else diff = (halo_val - cuVEC<VType>::quantity[idx]) / hsq;
			}
		}
		...
		if (ngbrFlags2[idx] & NF2_HALOPZ) {
			...
			if (ngbrFlags[idx] & NF_NNZ) diff = (halo_val + cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x*cuVEC<VType>::n.y] - 2 * cuVEC<VType>::quantity[idx]) / hsq;
				...
				if (ngbrFlags2[idx] & NF2_ROBINV) {
					...
					diff = ((1 + robin_v.i * cuVEC<VType>::h.z / (2 * K)) * halo_val + robin_v.i * cuVEC<VType>::h.z * robin_v.j / K - (1 + 3 * robin_v.i * cuVEC<VType>::h.z / (2 * K)) * cuVEC<VType>::quantity[idx]) / hsq;
				}
				else diff = (halo_val - cuVEC<VType>::quantity[idx]) / hsq;
			}
		}
