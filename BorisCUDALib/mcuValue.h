...
public:
	...
	//get reference to managed object pointer on given device : remember this is still in gpu memory; useful if we used with a gpu_to_... method from alloc_cpy.h to store in a cu_arr
	VType*& get_managed_object(int idx)
	{
		return pcuval[idx]->get_managed_object();
	}
