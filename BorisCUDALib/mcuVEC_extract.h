...
bool mcuVEC<VType, MType>::extract_profile_component_x(cuReal3 start, cuReal3 end, cuBReal step, cuReal3 stencil, mcu_arr<cuReal2>& profile_gpu)
{
	...
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
		...
		if (prect_d[mGPU].intersection_test(start + rect.s, end + rect.s, &intersection_s, &intersection_e)) {
			...
			//extract profile on this device
			//coordinates now must be relative to subrect
			if (!mng(mGPU)->extract_profile_component_x(intersection_s + rect.s - prect_d[mGPU].s, intersection_e + rect.s - prect_d[mGPU].s, step, stencil, profile_gpu.get_cu_arr(mGPU), (intersection_s - start).norm())) return false;
		}
		else {
...
bool mcuVEC<VType, MType>::extract_profile_component_x(cuReal3 start, cuReal3 end, cuBReal step, cuReal3 stencil, std::vector<DBL2>& profile_cpu)
{
	...
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
		...
		if (prect_d[mGPU].intersection_test(start + rect.s, end + rect.s, &intersection_s, &intersection_e)) {
			...
			//extract profile on this device to auxiliary storage first
			//coordinates now must be relative to subrect
			if (!mng(mGPU)->extract_profile_component_x(intersection_s + rect.s - prect_d[mGPU].s, intersection_e + rect.s - prect_d[mGPU].s, step, stencil, profile_component_aux.get_cu_arr(mGPU), (intersection_s - start).norm())) return false;
		}
		else {
...
bool mcuVEC<VType, MType>::extract_profile_component_y(cuReal3 start, cuReal3 end, cuBReal step, cuReal3 stencil, mcu_arr<cuReal2>& profile_gpu)
{
	...
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
		...
		if (prect_d[mGPU].intersection_test(start + rect.s, end + rect.s, &intersection_s, &intersection_e)) {
			...
			//extract profile on this device
			//coordinates now must be relative to subrect
			if (!mng(mGPU)->extract_profile_component_y(intersection_s + rect.s - prect_d[mGPU].s, intersection_e + rect.s - prect_d[mGPU].s, step, stencil, profile_gpu.get_cu_arr(mGPU), (intersection_s - start).norm())) return false;
		}
		else {
...
bool mcuVEC<VType, MType>::extract_profile_component_y(cuReal3 start, cuReal3 end, cuBReal step, cuReal3 stencil, std::vector<DBL2>& profile_cpu)
{
	...
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
		...
		if (prect_d[mGPU].intersection_test(start + rect.s, end + rect.s, &intersection_s, &intersection_e)) {
			...
			//extract profile on this device to auxiliary storage first
			//coordinates now must be relative to subrect
			if (!mng(mGPU)->extract_profile_component_y(intersection_s + rect.s - prect_d[mGPU].s, intersection_e + rect.s - prect_d[mGPU].s, step, stencil, profile_component_aux.get_cu_arr(mGPU), (intersection_s - start).norm())) return false;
		}
		else {
...
bool mcuVEC<VType, MType>::extract_profile_component_z(cuReal3 start, cuReal3 end, cuBReal step, cuReal3 stencil, mcu_arr<cuReal2>& profile_gpu)
{
	...
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
		...
		if (prect_d[mGPU].intersection_test(start + rect.s, end + rect.s, &intersection_s, &intersection_e)) {
			...
			//extract profile on this device
			//coordinates now must be relative to subrect
			if (!mng(mGPU)->extract_profile_component_z(intersection_s + rect.s - prect_d[mGPU].s, intersection_e + rect.s - prect_d[mGPU].s, step, stencil, profile_gpu.get_cu_arr(mGPU), (intersection_s - start).norm())) return false;
		}
		else {
...
bool mcuVEC<VType, MType>::extract_profile_component_z(cuReal3 start, cuReal3 end, cuBReal step, cuReal3 stencil, std::vector<DBL2>& profile_cpu)
{
	...
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
		...
		if (prect_d[mGPU].intersection_test(start + rect.s, end + rect.s, &intersection_s, &intersection_e)) {
			...
			//extract profile on this device to auxiliary storage first
			//coordinates now must be relative to subrect
			if (!mng(mGPU)->extract_profile_component_z(intersection_s + rect.s - prect_d[mGPU].s, intersection_e + rect.s - prect_d[mGPU].s, step, stencil, profile_component_aux.get_cu_arr(mGPU), (intersection_s - start).norm())) return false;
		}
		else {
...
template <typename VType, typename MType>
int mcuVEC<VType, MType>::extract_profile_component_max(cuReal3 start, cuReal3 end, cuBReal step, cuReal3 stencil, mcu_arr<cuReal2>& profile_gpu)
{
	if (mGPU.get_num_devices() == 1) {
	...
	//component should take value of 0 (x), 1 (y), 2 (z) once detected
	int component = -1;
	...
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
		...
		if (prect_d[mGPU].intersection_test(start + rect.s, end + rect.s, &intersection_s, &intersection_e)) {
			...
			case -1:
				//use this to detect which component to extract
				component = mng(mGPU)->extract_profile_component_max(intersection_s + rect.s - prect_d[mGPU].s, intersection_e + rect.s - prect_d[mGPU].s, step, stencil, profile_gpu.get_cu_arr(mGPU), (intersection_s - start).norm());
				if (component == -1) return component;
				break;

				//once component detected keep using same one
			case 0:
				if (!mng(mGPU)->extract_profile_component_x(intersection_s + rect.s - prect_d[mGPU].s, intersection_e + rect.s - prect_d[mGPU].s, step, stencil, profile_gpu.get_cu_arr(mGPU), (intersection_s - start).norm())) return -1;
				break;

			case 1:
				if (!mng(mGPU)->extract_profile_component_y(intersection_s + rect.s - prect_d[mGPU].s, intersection_e + rect.s - prect_d[mGPU].s, step, stencil, profile_gpu.get_cu_arr(mGPU), (intersection_s - start).norm())) return -1;
				break;

			case 2:
				if (!mng(mGPU)->extract_profile_component_z(intersection_s + rect.s - prect_d[mGPU].s, intersection_e + rect.s - prect_d[mGPU].s, step, stencil, profile_gpu.get_cu_arr(mGPU), (intersection_s - start).norm())) return -1;
				break;
			}
		}
		else {
...
//return extracted component : 0 (x), 1 (y), 2 (z), -1 (failed)
template <typename VType, typename MType>
int mcuVEC<VType, MType>::extract_profile_component_max(cuReal3 start, cuReal3 end, cuBReal step, cuReal3 stencil, std::vector<DBL2>& profile_cpu)
{
	if (mGPU.get_num_devices() == 1) {
	...
	for (mGPU.device_begin(); mGPU != mGPU.device_end(); mGPU++) {
		...
		if (prect_d[mGPU].intersection_test(start + rect.s, end + rect.s, &intersection_s, &intersection_e)) {
			...
			case -1:
				//use this to detect which component to extract
				component = mng(mGPU)->extract_profile_component_max(intersection_s + rect.s - prect_d[mGPU].s, intersection_e + rect.s - prect_d[mGPU].s, step, stencil, profile_component_aux.get_cu_arr(mGPU), (intersection_s - start).norm());
				if (component == -1) return component;
				break;

				//once component detected keep using same one
			case 0:
				if (!mng(mGPU)->extract_profile_component_x(intersection_s + rect.s - prect_d[mGPU].s, intersection_e + rect.s - prect_d[mGPU].s, step, stencil, profile_component_aux.get_cu_arr(mGPU), (intersection_s - start).norm())) return -1;
				break;

			case 1:
				if (!mng(mGPU)->extract_profile_component_y(intersection_s + rect.s - prect_d[mGPU].s, intersection_e + rect.s - prect_d[mGPU].s, step, stencil, profile_component_aux.get_cu_arr(mGPU), (intersection_s - start).norm())) return -1;
				break;

			case 2:
				if (!mng(mGPU)->extract_profile_component_z(intersection_s + rect.s - prect_d[mGPU].s, intersection_e + rect.s - prect_d[mGPU].s, step, stencil, profile_component_aux.get_cu_arr(mGPU), (intersection_s - start).norm())) return -1;
				break;
			}
		}
		else {
