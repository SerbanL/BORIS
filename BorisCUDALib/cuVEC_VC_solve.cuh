		...
		//use halo region?
			...
			if (ngbrFlags2[idx] & NF2_HALOPX) {
				...
				if (ngbrFlags[idx] & NF_NNX) {
					...
					total_weight += 2 * w_x;
					weighted_sum += w_x * (cuVEC<VType>::quantity[idx - 1] + halo_val);
				}
				else {

					total_weight += w_x;
					weighted_sum += w_x * halo_val;
				}
			}
			else {
				...
				if (ngbrFlags[idx] & NF_NPX) {
					...
					total_weight += 2 * w_x;
					weighted_sum += w_x * (halo_val + cuVEC<VType>::quantity[idx + 1]);
				}
				else {
		...
		//use halo region?
			...
			if (ngbrFlags2[idx] & NF2_HALOPY) {
				...
				if (ngbrFlags[idx] & NF_NNY) {
					...
					total_weight += 2 * w_y;
					weighted_sum += w_y * (cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x] + halo_val);
				}
				else {

					total_weight += w_y;
					weighted_sum += w_y * halo_val;
				}
			}
			else {
				...
				if (ngbrFlags[idx] & NF_NPY) {
					...
					total_weight += 2 * w_y;
					weighted_sum += w_y * (halo_val + cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x]);
				}
				else {
		...
		//use halo region?
			...
			if (ngbrFlags2[idx] & NF2_HALOPZ) {
				...
				if (ngbrFlags[idx] & NF_NNZ) {
					...
					total_weight += 2 * w_z;
					weighted_sum += w_z * (cuVEC<VType>::quantity[idx - cuVEC<VType>::n.x*cuVEC<VType>::n.y] + halo_val);
				}
				else {

					total_weight += w_z;
					weighted_sum += w_z * halo_val;
				}
			}
			else {
				...
				if (ngbrFlags[idx] & NF_NPZ) {
					...
					total_weight += 2 * w_z;
					weighted_sum += w_z * (halo_val + cuVEC<VType>::quantity[idx + cuVEC<VType>::n.x*cuVEC<VType>::n.y]);
				}
				else {
		...
		//use halo region?
			...
			if (ngbrFlags2[idx] & NF2_HALOPX) {
				...
				if (ngbrFlags[idx] & NF_NNX) {
					...
					total_weight += w_x;
					weighted_sum += w_x * (halo_val - obj.bdiff(idx).x * cuVEC<VType>::h.x);
				}
			}
			else {
		...
		//use halo region?
			...
			if (ngbrFlags2[idx] & NF2_HALOPY) {
				...
				if (ngbrFlags[idx] & NF_NNY) {
					...
					total_weight += w_y;
					weighted_sum += w_y * (halo_val - obj.bdiff(idx).y * cuVEC<VType>::h.y);
				}
			}
			else {
		...
		//use halo region?
			...
			if (ngbrFlags2[idx] & NF2_HALOPZ) {
				...
				if (ngbrFlags[idx] & NF_NNZ) {
					...
					total_weight += w_z;
					weighted_sum += w_z * (halo_val - obj.bdiff(idx).z * cuVEC<VType>::h.z);
				}
			}
			else {
